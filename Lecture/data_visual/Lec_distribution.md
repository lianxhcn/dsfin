...
> **本章目标**：理解直方图、核密度估计、箱线图和小提琴图的统计原理，能够根据数据特征选择合适的图形，并正确解读分布的形态特征。本章是第4章可视化工具的"理论底座"，也是后续计量建模（正态性假设、异方差检验、残差分析）的基础。
...
| Epanechnikov 核 | $K(u) = \frac{3}{4}(1-u^2)\mathbf{1}(|u|\leq1)$ | 有限支撑，均方误差最优 |
| 均匀核 | $K(u) = \frac{1}{2}\mathbf{1}(|u|\leq1)$ | 等价于滑动平均直方图 |
