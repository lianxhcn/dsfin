...
log using ${logdir}ami13.log, replace

*=================================================================================
*AMI
#delimit ;

guse ${datadir1}ip_ami_predict_year_er.dta;
...
*There are 12,694 cells in the ER AMI sample, where we exclude hospital-quarters with 0 invasive procedures;

compress;

forvalues x=12001/12694 {;
...
replace beta_ang=_b[xb_overall] if new_id==`x';
replace se_beta_ang=_se[xb_overall] if new_id==`x';
...
replace beta_ca_ang=_b[xb_ca] if new_id==`x';
replace se_beta_ca_ang=_se[xb_ca] if new_id==`x';
...
replace beta_im_ang=_b[xb_im] if new_id==`x';
replace se_beta_im_ang=_se[xb_im] if new_id==`x';
...
replace beta_inv=_b[xb_overall] if new_id==`x';
replace se_beta_inv=_se[xb_overall] if new_id==`x';
...
replace beta_ca_inv=_b[xb_ca] if new_id==`x';
replace se_beta_ca_inv=_se[xb_ca] if new_id==`x';
...
replace beta_im_inv=_b[xb_im] if new_id==`x';
replace se_beta_im_inv=_se[xb_im] if new_id==`x';
...
gsave ${datadir2}ip_betas_13000_ami_er.dta, replace;
