...
爱德华·塔夫特（Edward Tufte）在《定量信息的视觉呈现》中提出了**数据墨水比（Data-Ink Ratio）**的概念：
...
数据可视化的误导，有时是无意的（设计不规范），有时是刻意的（为了强化某个结论）。识别误导图形是数据素养的重要组成部分。

**① 截断 Y 轴夸大差异**

财经媒体和上市公司公告中最常见。某公司净利润从 100 亿增长到 103 亿，Y 轴从 98 亿开始，柱体看起来增长了 150%，实际只有 3%。判断方法：看 Y 轴的起点，问自己"如果 Y 轴从零开始，这个差异还显著吗？"

**② 选择性时间窗口**

选择对自己有利的起始和结束时间点。某产品宣称"过去三年累计收益 60%"，但恰好从市场底部算起。判断方法：拉长时间窗口，或选择多个不同起始点重新计算。

**③ 面积/体积误用**

用圆的面积代表数值大小时，有些图用**半径**而非**面积**对应数值——导致大值被严重夸大（面积 = π r²，半径翻倍则面积变为 4 倍）。

**④ 用双 Y 轴制造虚假相关性**

通过分别调整两个 Y 轴的范围，可以让任意两条时间序列看起来高度相关或完全无关。

**⑤ 忽略基数的百分比变化**

"同比增长 100%"可能是从 1 单位增长到 2 单位。绝对值和相对值应该同时呈现。

> 📌 **建议**：在阅读财经报告、研究报告时，养成习惯——先看坐标轴范围，再看数据本身。

::: {.callout-tip}
#### 示例提示词

```
请生成两张对比图，演示「截断 Y 轴」的误导效果：
- 数据：5 个季度的营收数据，数值依次为 [100, 101, 102, 103, 104]（单位：亿元）
- 左图：Y 轴从 0 开始（诚实版本）
- 右图：Y 轴从 99 开始（误导版本）
- 两张图使用相同的标题、颜色和样式，只有 Y 轴范围不同
- 在图形下方添加说明文字，指出两图的视觉差异
```
:::
...
- 张蛟蛟 (2025). 从"能看"到"好懂"：科研图表设计的原则和建议. 连享会.
