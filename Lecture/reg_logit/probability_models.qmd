...
#### Q1. 二元变量 (Binary Variables) {.unnumbered}
...
#### Q2. 多项类别变量 (Multinomial Variables) {.unnumbered}
...
#### Q3. 有序变量 (Ordered Variables) {.unnumbered}
...
#### C3. 截断变量 (Truncated Variables) {.unnumbered}
...
#### C4. 归并变量 (Censored Variables) {.unnumbered}
...
#### C5. 计数变量 (Count Variables) {.unnumbered}
...
## 二元选择模型：Logit/Probit {#sec-logit-model}
...
$$p(x) = \mathrm{Pr}(y_i=1\mid \mathbf{x}) = G(\mathbf{x}_i'\beta)$$ {#eq-MLE-eg-05}
...
[^8]: 换个角度来看，这也是一种降维的方法。如果直接估计 $\theta_i$，则会有 $N$ 个待估参数，但通过链接函数 $G(\mathbf{x}_i'\beta)$ 我们就只需估计 $k$ ($k$ 是 $\mathbf{x}$ 中包含的解释变量的个数) 个参数。

-   LPM：

$$p(x) = \mathbf{x}_i^\prime \beta$$ {#eq-logit-link-01}

-   Logit：

$$
p(x) = \Lambda(\mathbf{x}_i^\prime \beta) =  \frac{\mathrm{exp}(\mathbf{x}_i^\prime \beta)}{1+\mathrm{exp}(\mathbf{x}_i^\prime \beta)}
$$ {#eq-logit-link-02}
...
$$
p(x) = \Phi(\mathbf{x}_i^\prime \beta) = \frac{1}{\sqrt{2 \pi}} \int_{-\infty}^{\mathbf{x}_i^\prime \beta} e^{{t^2}/2} d t
$$ {#eq-logit-link-03}
...
5.  预测概率。 一旦获得参数估计值 $\hat{\beta}$，只需用其替代 (@eq-MLE-eg-05) 式中的 $\beta$，即可得到概率估计值： $$\widehat{p} (x) = G (\mathbf{x}_i^\prime \hat{\beta})$$
...
6.  边际效应。多数情况下，链接函数 $G(\cdot)$ 都是非线性函数，从而导致 $x$ 对 $p(x)$ 的边际影响不再是常数，但我们可以基于 (@eq-prob-basic-10) 式计算边际效应。具体到二元选择模型，边际效应定义如下：

$$
\mathrm{ME} = \frac{\partial p(x)}{\partial x}=G^{\prime}(\mathbf{x}_i^\prime \hat{\beta}) \beta
$$ {#eq-logit-ME-00}
...
$$
\mathrm{ME}_{logit}  =  \frac{\mathrm{exp}(\mathbf{x}_i^\prime \beta_{Logit})}{[1+\mathrm{exp}(\mathbf{x}_i^\prime \beta_{Logit})]^2}\beta_{Logit}
$$ {#eq-logit-ME-02}

-   Probit：

$$\mathrm{ME}_{Probit} = \phi(\mathbf{x}_i^\prime \beta_{Probit})\beta_{Probit}$$ {#eq-logit-ME-03}
...
#### B. Probit 和 Logit 模型 {.unnumbered}
...
当然，当我们用 `regress`, `logit`, `probit` 分别估计 PLM, Logit 和 Probit 模型时，三者的系数值不可以直接比较，这从 (@eq-logit-ME-01)-(@eq-logit-ME-03) 式可以看出：三者的 ME 相近，但由于 $\beta$ 前面的乘数不同，必然导致 $\beta_{PLM}$ 与 $\beta_{Logit}$ 以及 $\beta_{Probit}$ 不同。经验上，三组系数的关系如下：

1.  $\beta_{PLM}$ 反映了平均边际效应，大体上与

![LPM，Probit 和 Logit 模型的链接函数](./figs/Logit-Link-function-01.png){#fig-Logit-Link-function-01 width="100%"}
...
其中，$x\sim U(0,10)$ 是服从均匀分布的随机数，取值介于 0 和 10 之间。 由 (@eq-Hansen2021-eq25.3a) 式可知，$P(x) = Pr(Y=1\mid x) = \Phi(x'\beta)$
...
tw (scatter ystar x if ystar>0, ytitle(y*)) ///
   ...
   (scatter y     x if ystar<0, msize(*0.5) yaxis(2)) ///
   lfit ystar x, ///
   legend(order(1 "y*" 2 "y*" 4 "y" 5 "y") ring(0) pos(5))
```
...
![$Y^*$ 和 $Y$ 的散点图。<br>其中，$Y^* = \mathbf{x}'\beta = -3 + 0.6 x + e$，$Y=\mathbf{1}(Y^*>0)$](./figs/Logit-Latent-variable-01.png){#fig-Logit-Latent-variable-01 width="100%"}

![$Y$ 的分仓散点图 (`binscatter`)。<br>制作步骤为：(1) 将样本按照 $x$ 由小到大排序，进而将所有观察值等分 20 组，每组的样本数相同；(2) 在每个子样本组中，计算 $\bar{Y}_j (j=1,2,\cdots,20)$，同理可得 $\bar{x}_j$；(3) 绘制 $\bar{Y}_j$ 对 $\bar{x}_j$ 的散点图。图中的空心圆圈表示每个子样本组中 $y=1$ 的占比，也即 $\mathrm{Pr}(y=1\mid x)$。](./figs/Logit-Latent-variable-02.png){#fig-Logit-Latent-variable-02 width="100%"}
...
按照此前的思路，我们可以对参数 $\theta$ 进行线性化设定以便分析影响 $\theta$ 的因素。考虑到 $\theta>0$，设定 $\theta_i = \exp(\mathbf{x}_i'\boldsymbol{\beta})$，用其替换 (@eq-prob-basic-poisson01) 式中的 $\theta$ 即可得到泊松回归模型的对数似然函数：

$$
\ln L_{n}\left(y_{i}, \mathbf{x}_i; \boldsymbol{\beta}\right)=-n \exp(\mathbf{x}_i'\boldsymbol{\beta})+ (\mathbf{x}_i'\boldsymbol{\beta}) \sum_{i}^{n} y_{i}-\sum_{i}^{n} \ln y_{i} !
...
![似然函数 $L(\theta)$ 与对数似然函数 $\mathrm{ln} L(\theta)$ 对比](./figs/B1_GLM_MLE_eg_01.png){#fig-B1_GLM_MLE_eg_01 width="100%"}
...
\begin{aligned}
 \ln {L}_j &=& \ln \frac{1}{\sqrt{2\pi \sigma^2}}
         \exp \left[-\frac{1}{2}\frac{(y_j-\mathbf{x}_j\beta)}{\sigma^2} \right] \\
   &=& \ln \left[ \frac{1}{\sigma}\cdot \phi\left( \frac{(y_j-\mathbf{x}_j\beta)}{\sigma} \right) \right]
\end{aligned}
$$
...
\begin{aligned}
  ...
  \theta_{1j} &=& \mu_j = \mathbf{x}_j\beta \\
  \theta_{2j} &=& \sigma
\end{aligned}
$$
...
\ln {L}_i =\left\{
  \begin{array}{l l}
     \ln \Phi(\mathbf{x}_j \boldsymbol{\beta})   & \mbox{若}~~ y_j = 1 \\
     \ln \Phi(-\mathbf{x}_j \boldsymbol{\beta})  & \mbox{若}~~ y_j = 0
  \end{array}
  \right.
$$ {#eq-mle_probit}
...
\small
 ...
 \nonumber  \theta_{2j} &=& \ln \gamma_j = \mathbf{x}_{2j} \beta_2
\end{aligned}
$$ {#eq-mle_weibull01}

其中， $t_j$ 表示事件发生的时间，若事件发生，则 $d_j = 1$，否则 $d_j =
0$。同时，$\ln \eta_j$ 和 $\ln \gamma_j$ 两个参数分别设定为 $\mathbf{x}_{1j}$ 和 $\mathbf{x}_{2j}$ 的线性函数。设定对数似然函数的 Stata 程序为：
...
$$
   u_{it}  \sim  \mbox{i.i.d.} \,\mbox{Exp}\, (\sigma_u), \quad  w_{it}  \sim  \mbox{i.i.d.} \,\mbox{Exp}\, (\sigma_w)
$$ {#eq-5205}
...
$$
\ln {L}(\mathbf{x}_{it};\boldsymbol{\theta}) = -\ln(\sigma_u+\sigma_w) +
