...
Lasso 的一个主要用途是解决「高维数据问题」，\footnote{%
   Belloni et al. (2014a) 对此类问题进行了梳理，并提供了几篇论文的重现资料。}
主要包括两种情形：
%\begin{itemize}
%\item 
...
在经济金融领域，Lasso 主要应用于第二种情形。以因果推断中最为常用的「反事实框架」为例，我们需要估计 (预测) 出实验个体在假想其不受政策干预的情况下的反事实结果 $y_0$，才能估计出政策效果 $\tau = y_1 - y_0$。借助 Lasso，我们可以筛选出具有最佳样本外预测能力的模型，从而得到反事实结果 $y_0$ 的估计值 $\widehat{y}_0$，进而得到 $\widehat{\tau}$。\footnote{%
      外部命令 \texttt{scul} 便采用 Lasso 方法估计了 Abadie et al. (2003, 2010) 提出的合成控制法。}
...
其二，「稀疏性假设」。多数情况下，虽然数据中有很多变量，但真正有用 (有助于样本外预测) 的变量却非常有限。这就是所谓的稀疏性，即模型中具有非零系数的变量只占很小的比例。我们的任务就是通过各种方法找出这些「有用」的变量。这个过程也称为「降维」。\footnote{%
      例如，在 Abadie et al. (2010) 研究加州禁烟政策的案例中，他们用 38 个在样本期内没有实施过禁烟政策的州作为实验个体 (加州) 的潜在控制对象，并利用这些州的加权平均虚构出一个与真实加州颇为相似的「合成加州」，进而用「加州」与「合成加州」的人均香烟消费量之差来衡量政策效果。事实上，最终只有 5 个州具有非零权重，换言之，只需用这 5 个州的信息就可以很好地预测加州的香烟消费量。}
对于高维数据，通常假设其满足稀疏性假设。更为宽松的是「近似稀疏性假设\index{稀疏性假设}\label{page:稀疏性假设}」：部分 $\beta_{j}$ 系数近似于零 (不一定等于零)，并且近似误差足够“小”。
...
\iffalse  %todo    稀疏性定义
高维数据的惩罚回归模型都依赖于一个基本的假设：稀疏性假设 (sparsity assumption) —— 在 $p$ 个潜在的解释变量中，只有 $s$ 个解释变量的系数不为零。换言之，虽然我们有很多信息 (变量)，但只有一小部分信息真正有用。严谨的表述如下：
$$
s:=\sum_{j=1}^{p} \mathbf{1}\left\{\beta_{j} \neq 0\right\} \ll n
$$
难点在于，我们并不知道哪些变量的系数为零，哪些不为零。这也是使用惩罚回归的主要原因，我们试图找到一些规则，以便于把那些不重要的变量剔掉。 
\fi

其三，「正则化」\index{正则化}和「惩罚回归」。所谓正则化，按照英文单词 regularization 的字面意思是“the condition of having been made regular (or more regular)”，也就说把“不正常的”模型向“正常的”或“比较正常的”状态调整。“不正常”的典型表现就是当模型中的变量高度共线性或 $p>n$ 时，$\X'\X$ 呈现病态 (如不可逆)，致使 OLS 估计很不稳定。修正的基本思路是对 OLS 的系数施加约束，使其估计结果趋于稳定，以便预测和外推。施加约束的过程可以视为对原有估计量的惩罚，这也是把 Lasso 回归和岭回归等统称为「惩罚回归 (penalized regression)」的原因。惩罚回归的结果通常都是有偏估计，这是我们为了降低估计的方差 (提高估计的准确性) 而必须付出的代价。  
...
介绍 Lasso 的书籍和综述文章很多，本章内容主要参考了 Hastie et al. (\href{https://web.stanford.edu/~hastie/StatLearnSparsity_files/SLS.pdf}{2015}) 和 Hansen (\href{https://www.ssc.wisc.edu/~bhansen/econometrics/Econometrics.pdf}{2021})。其它推荐阅读的资料还包括：Athey and Imbens (\href{http://sci-hub.ren/10.1146/annurev-economics-080217-053433}{2019}), Ahrens et al. (\href{http://sci-hub.ren/10.1177/1536867X20909697}{2020})，Belloni et al. (\href{https://doi.org/10.1257/jep.28.2.29}{2014a})，Cerulli (\href{https://arxiv.org/pdf/2103.03122.pdf}{2021b})，以及 Fan et al. (\href{http://sci-hub.ren/10.1201/9780429096280}{2021}) 等。\footnote{%
Ahrens, A., C. B. Hansen, and M. E. Schaffer. 2020. lassopack: Model selection and prediction with regularized regression in Stata. Stata Journal 20: 176–235. \href{http://sci-hub.ren/10.1177/1536867X20909697}{PDF}, \href{https://statalasso.github.io/pdf/StataConference2018_AhrensSchaffer.pdf}{PPT}

Athey, S., \& Imbens, G. W. (2019). Machine learning methods that economists should know about. Annual Review of Economics, 11, 685-725. \href{https://doi.org/10.1146/annurev-economics-080217-053433}{Link}, \href{http://sci-hub.ren/10.1146/annurev-economics-080217-053433}{PDF}

Belloni, A., V. Chernozhukov, and C. B. Hansen. 2014a. High-dimensional methods and inference on structural and
treatment effects. Journal of Economic Perspectives 28: 29–50. \href{https://doi.org/10.1257/jep.28.2.29}{Link}, \href{http://sci-hub.ren/10.1257/jep.28.2.29}{PDF}, \href{https://academic.oup.com/restud/article/81/2/608/1523757}{Stata-codes}

Cerulli, G., 2021b. "Machine learning using Stata/Python," 2021 Stata Conference 25, Stata Users Group. \href{https://arxiv.org/pdf/2103.03122.pdf}{PDF}

Fan, Jianqing, Runze Li, Cun-Hui Zhang, and Hui Zou. 2020. Statistical Foundations of Data Science. 1st Edition, Chapman and Hall/CRC. \href{https://doi.org/10.1201/9780429096280}{Link}, \href{http://sci-hub.ren/10.1201/9780429096280}{PDF}

Hansen B E . 2021. Econometrics. Princeton University Press. \href{https://www.ssc.wisc.edu/~bhansen/econometrics/}{Data and Contents}, \href{https://www.ssc.wisc.edu/~bhansen/econometrics/Econometrics.pdf}{PDF}

Hastie, T. J., R. J. Tibshirani, and M. Wainwright. 2015. Statistical Learning with Sparsity: The Lasso and
Generalizations. Boca Raton, FL: CRC Press. \href{https://web.stanford.edu/~hastie/StatLearnSparsity/}{Link}. \href{https://web.stanford.edu/~hastie/StatLearnSparsity_files/SLS.pdf}{PDF} %
}
\vspace{10pt}
...
给定一组样本，假设待估参数为 $\theta$，
其真实值为 $\theta_0$，估计值为 $\hat{\theta}$，二者之差反映了估计值与真实之间偏离程度，记为  $D(\hat{\theta}) =\hat{\theta}-\theta_0$。考虑到 $\hat{\theta}$ 是一个随机数，我们可以对 $[D(\hat{\theta})]^2$ 取期望，这便是所谓的“均方误差” (mean-square error, MSE)：
$$
\operatorname{MSE}(\hat{\theta})=\mathrm{E}\,[D(\hat{\theta})^2] = \mathrm{E}\left[(\hat{\theta}-\theta_0)^2\right]
$$
乍看起来，MSE 有点像估计量的方差，但其实二者不同。估计量的方差定义为：
$$
\operatorname{Var}(\hat{\theta}) = \mathrm{E}\left[\hat{\theta}-\mathrm{E}(\hat{\theta})\right]^{2}
$$
可以看出，$\operatorname{Var}(\hat{\theta})$ 表示估量 $\hat{\theta}$ 与其期望值 $\mathrm{E}(\hat{\theta})$ 之间的偏离程度，即“估计值的不确定性”；而 $\operatorname{MSE}(\hat{\theta})$ 则表示估计量 $\hat{\theta}$ 与其真实值 $\theta_0$ 之间的偏离程度。另一个重要的指标是估计量的偏差：
$$ \operatorname{Bias}(\hat{\theta})=\mathrm{E}(\hat{\theta})-\theta_{0}
$$
它表示估计量的期望值与真实之间的偏离程度，或曰“平均而言，$\hat{\theta}$ 与 $\theta_0$ 之间差多少”。

显然，在多个备选估计量中，MSE 最小者为佳，\footnote{%
    比如，对于存在截面异质性的面板数据模型，$\hat{\theta}$ 可以是混合 OLS 估计量 $\widehat{\beta}_{\mathrm{OLS}}$，也组内估计量 $\widehat{\beta}_{\mathrm{FE}}$ 或一阶差分估计量 $\widehat{\beta}_{\mathrm{FD}}$。此外，符号 $E (\hat{\theta}-\theta_{0})^{2}$ 表示 $E[(\hat{\theta}-\theta_{0})^{2}]$ 而非 $[E(\hat{\theta}-\theta_{0})]^{2}$。%
  }
但它却是一个在估计量的偏差 $\operatorname{Bias}(\hat{\theta})$ 和方差 $\operatorname{Var}(\hat{\theta}) $ 之间权衡的结果：\footnote{%
  证明：
%  $$
%  \begin{aligned}
%  \operatorname{MSE}(\hat{\theta})=& E\left(\hat{\theta}-\theta_{0}\right)^{2} \\
%  =& E\left([\hat{\theta}-E(\hat{\theta})]+\left[E(\hat{\theta})-\theta_{0}\right]\right)^{2} \\
%  =& E[\hat{\theta}-E(\hat{\theta})]^{2}+E\left[E(\hat{\theta})-\theta_{0}\right]^{2}+\\
%  & 2 \cdot E\left([\hat{\theta}-E(\hat{\theta})]\left[E(\hat{\theta})-\theta_{0}\right]\right) \\
%  =& \operatorname{Var}(\hat{\theta})+\operatorname{Bias}(\hat{\theta})^{2}
%  \end{aligned}
%  $$
  $$
  \begin{aligned}
  \operatorname{MSE}(\hat{\theta})=& E(\hat{\theta}-\theta_{0})^{2} \\
  =& E([\hat{\theta}-E(\hat{\theta})]+[E(\hat{\theta})-\theta_{0}])^{2} \\
  =& E[\hat{\theta}-E(\hat{\theta})]^{2}+E[E(\hat{\theta})-\theta_{0}]^{2}+ 2 \cdot E([\hat{\theta}-E(\hat{\theta})][E(\hat{\theta})-\theta_{0}]) \\
  =& \operatorname{Var}(\hat{\theta})+\operatorname{Bias}(\hat{\theta})^{2}
  \end{aligned}
  $$
  由于 $[E(\hat{\theta})-\theta_{0}]$ 是一个常数，我们可以把它从期望中取出，因此
  $
  E([\hat{\theta}-E(\hat{\theta})][E(\hat{\theta})-\theta_{0}]) =[E(\hat{\theta})-\theta_{0}] \cdot E[\hat{\theta}-E(\hat{\theta})] 
  =0
  $
}
\begin{equation}\label{eq:Lasso-01}
	\operatorname{MSE}(\hat{\theta})=\operatorname{Bias}(\hat{\theta})^{2}+\operatorname{Var}(\hat{\theta})
\end{equation}

图 \ref{Fig:Lasso-01} 非常直观地展示了二者之间的关系。显然，左上角的情形对应的 MSE 最小，而右下角的情形中 MSE 最大。

\begin{figure}
%todo %todo Source: Wasserman-2006-all_of_nonparametric_statistics， pp.50
	\centering
	\includegraphics[width=1\textwidth]{Lasso_variance_bias_01.png} 
	\caption{预测「偏差-方差」的权衡关系。\\
	说明：黑色正方形小点表示真实值，而空心圆圈表示估计值。该图表明，相对于「低偏差-高方差」估计量而言，「高偏差-低方差」估计量的预测结果可能更接近真实情况。} %caption是图片的标题
	\label{Fig:Lasso-01} 
\end{figure}

大家可以想象一下，若让一个“烂枪手” (偏差大：总是瞄偏) 拿着一把“破枪” (方差大：精度和可靠性都不行) 来发射 20 发子弹，会对应图中的哪种情形呢？

通常，我们会把样本分成两部分，一部分 (比如占 $70\%$) 用于估计模型参数，另一部分 (比如占 $30\%$) 用于检验模型的预测能力。前者称为「训练样本」或「训练集」，后者称为「测试集」或「验证集」。

评价一个模型的好坏，不但要看训练误差，更要关注测试误差，后者是很多研究的重点。显然，随着模型复杂度的提高，训练误差会不断降低，甚至为零，但这并不意味着测试误差也一定小。通常而言，预测误差与模型的复杂度之间存在“U 型”关系，也就是说，复杂度适中的模型最好。见图 \ref{Fig:Lasso-02} 和图 \ref{Fig-Lasso-bias-trade-01} 。

\begin{figure}%[H]
	\centering
	\includegraphics[width=.9\textwidth]{Fig-Lasso-Bias-variance-tradeoff-02.png} 
	\caption{预测「偏差-方差」的权衡关系。\\
	训练误差和测试误差与模型复杂度的关系。下方的蓝色细线表示训练误差 (样本内)；上方的红色细线表示测试误差 (样本外)。粗实线表示平均值 (每组包含 50 个观察值，重复做 100 次)。\\
	Source: Hastie et al. (2017), The Elements of Statistical Learning. Fig 7.1.}
%	Behavior of test sample and training sample error as the model complexity is varied. The light blue curves show the training error $\mathrm{err}$, while the light red curves show the conditional test error $\operatorname{Err}_{\mathcal{T}}$ for 100 training sets of size 50 each, as the model complexity is increased. The solid curves show the expected test error Err and the expected training error $\mathrm{E}[\mathrm{err}]$. %caption是图片的标题
	\label{Fig:Lasso-02} 
\end{figure}

\begin{figure}
	\centering
	\includegraphics[width=1.0\textwidth]{Lasso_variance_bias_02.png} 
	\caption{方差与偏差的权衡}
	\label{Fig-Lasso-bias-trade-01} 
	%，\href{https://www.analyticsvidhya.com/blog/2017/06/a-comprehensive-guide-for-linear-ridge-and-lasso-regression/#h2_16}{Source}
\end{figure}

\subsubsection{过拟合问题}

所谓「过拟合」，是指训练误差很小，但测试误差很大、甚至没有任何预测能力的情形。%原因在于，在不断增加模型复杂度以降低训练误差的过程中，这个复杂的模型捕捉了太多“噪音成分”，无法抓住问题的本质。
增加模型复杂度虽然有助于降低训练误差，但也会过多地捕捉“噪音成分”，无法抓住问题的本质，致使样本外预测能力很弱。

大家可以思考一下，对于图 \ref{Fig:Lasso-03} 所示的样本而言，何种情况下会出现过拟合问题？

\begin{figure}
	\centering
	\includegraphics[width=1.0\textwidth]{Fig-Lasso-overfit.png} 
	\caption{过拟合问题}
	\label{Fig:Lasso-03} 
\end{figure}

图 \ref{fig:Lasso-04} 展示了 Abadie et al. (\href{http://www.jenshainmueller.de/Paper/ccs.pdf}{2010}) 禁烟案例中的时间安慰剂检验结果，前者存在过拟合问题。

在这个案例中，加州 (California) 于 1989 年实施了禁烟法案，它在 1970-2000 年期间的人均香烟消费量由图中的蓝色实线表示，而虚线则表示反事实结果——由在样本期内始终未实施禁烟政策的其它 38 个州的加权平均计算而得。为了检验是否存在样本内过拟合问题，图 \ref{fig:Lasso-04-a} 和图 \ref{fig:Lasso-04-b} 分别采用合成控制法 (SCM) 和回归控制法 (RCM) 进行了时间安慰剂检验。虚构的事件时点为 1985 年。可以预期，若不存在过拟合，则在 1985-1989 年时段内 (政策尚未实施)，应该无法观察到明显的政策效果，即图中实线和虚线之间的偏差应该接近于 0。显然，图 \ref{fig:Lasso-04-a} 中的结果并不符合上述预期，即采用合成控制法存在过拟合问题，致使我们无法判断 1989 年之后图中实线与虚线之间的偏差到底是政策效应还是估计偏差。相比之下，图 \ref{fig:Lasso-04-b} 中基于回归控制法的估计结果则不存在过拟合问题。
...
\subfigure[SCM]{
  \includegraphics[width=\subFigWide]{Figs/Abadie10-Fig09-place-time.png}
  \label{fig:Lasso-04-a}
}
\quad
\subfigure[RCM (lasso+CV)]{
  \includegraphics[width=\subFigWide]{Figs/RCM-pred_pboTime1985-good}
  \label{fig:Lasso-04-b}
}
\caption{加州禁烟案例时间安慰剂检验}
\label{fig:Lasso-04}
\end{figure}
...
考虑如下线性模型：
\begin{equation}\label{eq:Lasso-02}
	y_i = \x_i'\betaz + e_i, \quad i=1,2,\cdots,n
\end{equation}
其中，$y_{i}$ 是被解释变量 (在机器学习中常被称为“标签”)，$\x_i$ 是由一组解释变量构成的向量，$\betaz$ 为系数向量。
若采用 OLS，则可以通过极小化残差平方和来估计参数，
\begin{equation}\label{eq:Lasso-03}
	\mathrm{RSS}(\beta)  
    =\sum_{i=1}^{n}\left(y_{i}-\x_{i}'\betaz\right)^{2} 
	= \sum_{i=1}^{n}e_i^{2}    
%	&=\sum_{i=1}^{n}\left(y_{i}-\hat{y}_{i}\right)^{2}
\end{equation}


在机器学习中，(\ref{eq:Lasso-03}) 通常称为「损失函数」(loss function) 或「代价函数」 (cost function)。在计量经济学中的习惯称谓“残差平方和最小”现在被简洁地称为“损失最小”。因此，所谓“损失”指的是 $y_i$ 中无法被模型解释的部分。
  
实际应用中，多基于均方误差 MSE 来定义损失函数：
\begin{equation}\label{eq:Lasso-04}
L(\beta)=\frac{1}{2 n} \sum_{i=1}^{n}e_i^{2}
\end{equation}
其中，加入 ${1}/{2}$ 是为了便于求导，而 ${1}/{n}$ 则是将损失平均化，以消除样本量 $n$ 的影响，保证不同样本之间的可比性。

对于离散选择模型 (如 Logit 模型)，多采用 MLE 估计，即通过极大化对数似然函数 $\ln L(\theta)$ 来求解参数 $\theta$。此时，只需以 $-\ln L(\theta)$ 为基础作适当调整即可定义损失函数。 


\subsection{范数与距离}\label{sec:Lasso-norm}

采用矩阵方式，线性回归模型通常写做：
$$
\y = \X \betaz + \e
$$
其中，$\e = (e_1,e_2,\cdots,e_n)'$ 为干扰项向量。
(\ref{eq:Lasso-04}) 式的精简表示为：
%\begin{equation}\label{eq:Lasso-06}
%L(\beta)=\frac{1}{2 n} \|\e\|_2^2 \quad \text{或} \quad L(\beta)=\frac{1}{2 n} \|\y - \X \betaz\|_2^2
%\end{equation}
\begin{equation}\label{eq:Lasso-06}
\quad L(\beta)=\frac{1}{2 n} \sum_{i=1}^{n}e_i^{2}
\quad \Longleftrightarrow \quad
L(\beta)=\frac{1}{2 n} \|\e\|_2^2 
\end{equation}
或
\begin{equation}\label{eq:Lasso-06-b}
L(\beta)=\frac{1}{2 n} \sum_{i=1}^{n}\left(y_{i}-\x_{i}'\betaz\right)^{2}
\quad \Longleftrightarrow \quad
L(\beta)=\frac{1}{2 n} \|\y - \X \betaz\|_2^2
\end{equation}
按照 (\ref{eq:Lasso-04}) 和 (\ref{eq:Lasso-06}) 的对应关系，不难发现：
$$\|\e\|_2^2 = e_1^2 + e_2^2 + \cdots + e_n^2$$
可见，$\|\e\|_2^2$ 表示向量 $\e$ 中各个元素平方和，它可以视为一个函数或标记符号，用于把向量 $\e$ 转换为一个度量其长度的标量。
换个角度来看，由于 $\e = \y - \X \betaz$，我们可以把 $\|\e\|_2^2$ 理解为一种定义向量 $\y$ 和向量 $\X \betaz$ 之间距离的方式。
%对比 (\ref{eq:Lasso-06}) 和 (\ref{eq:Lasso-06-b}) 可知，$\|\e\|_2^2 = \|\y - \X \betaz\|_2^2

下面简要介绍范数 (norm) 的概念，以便更为正式地介绍在机器学习中广泛应用的各种向量距离的衡量方式。

给定向量 $\x$，其范数的通用定义公式如下:
$$
\|\x\|_{p}=\left(\left|x_{1}\right|^{p}+\ldots+\left|x_{n}\right|^{p}\right)^{\frac{1}{p}} \quad p \geq 1, x \in \mathbb{R}^{n}
$$

\begin{itemize}
\item 
$\ell_{1}$ 范数 ($p=1$)：$\|\x\|_{1}=\sum_{i=1}^{N}\left|x_{i}\right|$。即向量各个元素的绝对值之和，又称为「曼哈顿距离」。使用 $\ell_{1}$ 范数可以度量两个向量间的差异 (绝对误差和)。
\item 
$\ell_{2}$ 范数 ($p=2$)：$\|\x\|_{2}=\sqrt{\sum_{i=1}^{N} x_{i}^{2}}$，亦称 Euclid范数 (欧几里得范数)。即向量各元素的平方和再开方，也可以表示向量的长度 (模) 或两个向量之间的距离，又称为「欧氏距离」。
\end{itemize}
...
\begin{itemize}
\item
$\ell_{0}$ 范数 ($p=0$)：指向量中非 0 的元素的个数，即 $\|\x\|_{0}=\sum_{i=1}^{n} \mathbf{1}\left\{x_{i} \neq 0\right\}$。%采用前向或反向逐步回归法筛选变量，并得到“最优子集 (best subset)”时会用到它。
比如，在 $p\!\gg\!n$ 的情况下，稀疏性假设可以表示为 $||\betaz||_0<n$。
\item  
$\ell_{\infty}$ 范数 (亦称 sup-norm)：指向量中绝对值最大的元素，即 $\|\x\|_{\infty}=\max _{1 \leq i \leq n}\left|x_{i}\right|$。
\item  
$\ell_{-\infty}$ 范数：指向量中绝对值最小的元素，即 $\|\x\|_{-\infty}=\min _{1 \leq i \leq n}\left|x_{i}\right|$
\end{itemize}

Lasso 中主要使用 $\ell_{1}$ 范数；岭回归会用到 $\ell_{2}$ 范数；而在最优子集筛选变量时则会用到 $\ell_{0}$ 范数。\footnote{%
  Stata 中，可以在 mata 环境下用 \texttt{norm()} 函数计算向量和矩阵的范数。 }



\iffalse
%todo Hansen 2021，Chap 29
\subsection{向量距离和范数}%$\ell_{1}_p$
For discussion of ridge and Lasso regression we will be making extensive use of the $\ell_{1}$-norm and 2norm, so it is useful to review the definition of the general $p$-norm. For a vector $a=\left(a_{1}, \ldots, a_{k}\right)^{\prime}$ the p-norm ( $p \geq 1)$ is
$$
\|a\|_{p}=\left(\sum_{j=1}^{k}\left|a_{j}\right|^{p}\right)^{1 / p}
$$
Important special cases include the $\ell_{1}$-norm
$$
...
$$
$\ell_{\infty}$-范式 (亦称 sup-norm) 
$$
\|a\|_{\infty}=\max _{1 \leq j \leq k}\left|a_{j}\right|
$$
We also define the $\ell_{0}$-norm
$$
\|a\|_{0}=\sum_{j=1}^{k} \mathrm{1}\left\{a_{j} \neq 0\right\}
$$
the number of non-zero elements. This is only heuristically labeled as a "norm".
The $\mathrm{p}$-norm satisfies the following additivity property. If $a=\left(a_{0}, a_{1}\right)$ then
$$
...
$$
\|a\|_{1} \geq\|a\|_{2} \geq\|a\|_{\infty} \text {. }
$$
Applying Hölder's (29.1) we also have the inequality
$$
\|a\|_{1}=\sum_{j=1}^{k}\left|a_{j}\right| \mathbf{1}\left\{a_{j} \neq 0\right\} \leq\|a\|_{2}\|a\|_{0}^{1 / 2}
$$
\fi
...
用以描述 $y$ 和 $\mathbf{x}$ 关系的备选模型很多，如简单线性回归模型、Poisson 模型、负二项回归模型等，区别在于对 $y_i$ 的分布特征的假设。\footnote{%
      例如，在简单线性回归模型中，通常假设 $y_i$ 服从正态分布；而在 Poisson 模型中则假设 $y_i$ 服从泊松分布。}
显然，不同的模型设定会有不同的预测能力，这就是所谓的「经验风险」，它本质上是选择何种模型的问题。
模型选定后，还需要考虑在模型中包含多少个参数的问题，这就是「结构风险」。\footnote{%
     例如，选定简单线性回归模型后，尚需考虑是否在模型中加入年龄的平方项，或年龄和性别以及职业的交叉项等模型设定问题。在此过程中，模型参数在增加，意味着结构风险在增大。 }显然，二者经常会纠缠在一起。
...
从损失函数的构造角度来看，所谓正则化项，描述的其实是模型的复杂度，它越高，过拟合的风险也就越大，它主要衡量结构风险。看起来，整个思路非常类似于从 $R^2$ 到 $\bar{R}^2$ 的演变，也类似于 AIC 和 BIC 等信息准则的构造过程。\footnote{%
      AIC 和 BIC 都试图在「拟合程度」和「模型复杂度」之间进行权衡。信息准则的通用表达式为：
        $$
        IC = -2\ln(\mathrm{likelihood}) + c(n, K)
        $$
      其中，$n$ 和 $K$ 分别为样本数和参数个数。模型的拟合程度用负数项 $-2\ln(\mathrm{likelihood})$ 来衡量，值越小越好；而模型复杂度则用取值为正数的惩罚项 $c(n, K)$ 来衡量。不同信息准则的差别在于惩罚力度不同。例如，对于 $\mathrm{AlC}$ 准则，$c=2 K$；而对于 $\mathrm{BIC}$ 准则，$c=K \log (n)$。详情参见第 \ref{sec:lasso-lambda-ic} 小节。
      }
...
基于中心化后的数据得到最优 Lasso 估计值 $\widehat{\beta}$ 后，可以恢复原数据的参数估计：$\widehat{\beta}$ 不变，常数项 的估计值 $\widehat{\beta}_{0}$ 如下：\footnote{%
  需要说明的是，该式只适用于以残差平方和为损失函数的线性回归。对于 Lasso-logit 等模型则不适用。}
...
\right\}
\end{equation}
其中，$\lambda \geq 0$，通常称为“调节参数”。由于 $\lambda$ 与 $\tau$ 的乘积 $\lambda \tau$ 是一个常数，因此，(\ref{eq:lasso-HTW2015-2-5-A}) 与 (\ref{eq:lasso-HTW2015-2-5}) 的极小化问题是等价的，\footnote{%
      此性质称为拉格朗日对偶性 (duality)。}
...
%对偶问题的
(\ref{eq:lasso-HTW2015-2-3}) 式的约束集 $\left\{\|\beta\|_{1} \leq \tau\right\}$ 是一个类似于多面钻石的交叉多面体 (见图 \ref{Fig-Lasso-3D-HTW2015front})。 二维情形下的最小化问题如图 \ref{Fig-Lasso-Lassopath-lian} 所示。椭圆形是 $\{\beta_1,\beta_2\}$ 在不同取值组合时的平方误差对应的等高线，OLS 估计处于该椭圆集的中心点上 (此时 $\beta_1$ 和 $\beta_2$ 均不为零)。\footnote{%todo 该图片随后删除，放入 OLS 那一章，在这里交叉引用
    图 \ref{fig:Lasso-ols-RSS} 呈现了两变量情形下，OLS 的 RSS 随参数变动的三维图，有助于理解图 \ref{Fig-Lasso-Lassopath-lian}。}
...
\iffalse %todo ================注释-------begin----
\begin{figure}
\centering
\begin{tikzpicture}[width=1.2\textwidth]
    \begin{scope}[line width=0.5pt,font=\scriptsize]
        \draw [->, color=black!30] (-1.8,0) -- (3.5,0) node [below left, color=black]  {$\beta_1$};
        \draw [->, color=black!30] (0,-1.8) -- (0,3.0) node [below right, color=black] {$\beta_2$};
    \end{scope}
    \path [draw=none,fill=gray!50,semitransparent] (1.5,0) -- (0,1.5) -- (-1.5,0) -- (0,-1.5) -- cycle;  %% 菱形
    \path [draw=line] (1.5,0) -- (0,1.5) -- (-1.5,0) -- (0,-1.5) -- cycle;  %% 菱形边界
    \draw[color=black] (0,0) circle (0.03); %%% (0,0)圆点
    \path [draw=none,fill=black] (1.3, 1.65) circle (0.03); %%% 椭圆中的圆点
    \draw[color=black] (1.56, 1.54) node [font=\tiny] {OLS}; 
    \draw[color=black,fill=black] (0.0, 1.5) circle (0.03); %%% 切点
    \draw[->,color=gray] (-0.5,1.35) node [left,font=\tiny,color=black] {Lasso} -- (-0.15,1.5) ;  
    \draw[->,color=gray] (1.8,2.6) node [right,font=\tiny,color=black] {Lasso path} -- (0.85,1.78) ;  
    % Place the equation into the circle:
    \node [below right, darkgray, font=\scriptsize] at (+1.47,-0.02) {$\tau$};
    \node [below right, darkgray, font=\scriptsize] at (+0.8,-0.7) {$|\beta_1|+|\beta_2| \leq \tau$};
    \draw[rotate=-15] (0.9, 1.928) ellipse (2 and 0.6) ;
    \draw[rotate=-15,color=gray] (0.9, 1.928) ellipse (1.6 and 0.4) ;
    \draw[rotate=-15,color=gray] (0.9, 1.928) ellipse (1.0 and 0.2) ;
    \draw [densely dashed, path=curve] (0,0) -- (0.0, 1.8) -- (1.3, 1.65) ;  %% 虚线
\end{tikzpicture}
	\caption{Lasso 求解过程图示 \\
	Source: Hansen (2021, Fig 29.1), 采用 Tikz 宏包重新绘制} %caption是图片的标题
	\label{Fig-Lasso-Ridge-figure3-Hansen2021} 
\end{figure}
\fi      %todo ================注释-------over-----
...
\caption{Lasso 和 Ridge 约束集对比 (\href{https://gitee.com/arlionn/LassoStata/wikis/Lasso-Ridge-regular.md}{Stata Codes})}
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
	\caption{Lasso 约束集} %caption是图片的标题
	\label{Fig:Lasso-tikz-02} 
\end{figure}
\fi      %todo ================注释-------over-----
...
One case where we can explicitly calculate the Lasso estimates is when the regressors are orthogonal. Suppose that $\boldsymbol{X}^{\prime} \boldsymbol{X}=\boldsymbol{I}_{p}$. Then the first order condition for minimization simplifies to
$$
-2\left(\widehat{\beta}_{\mathrm{ols}, j}-\widehat{\beta}_{\mathrm{Lasso}, j}\right)+\lambda \operatorname{sgn}\left(\widehat{\beta}_{\mathrm{Lasso}, j}\right)=0
$$
which has the explicit solution
$$
\widehat{\beta}_{\text {Lasso, } j}=\left\{\begin{array}{cc}
\widehat{\beta}_{\text {ols }, j}-\lambda / 2 & \widehat{\beta}_{\text {ols, } j}>\lambda / 2 \\
0 & \left|\widehat{\beta}_{\text {ols, } j}\right| \leq \lambda / 2 \\
\widehat{\beta}_{\text {ols }, j}+\lambda / 2 & \widehat{\beta}_{\text {ols }, j}<-\lambda / 2
\end{array}\right.
$$
This Lasso estimate is a continuous transformation of the least squares estimate. For small values of the least squares estimate the Lasso estimate is set to zero. For all other values the Lasso estimate moves the least squares estimate towards zero by $\lambda / 2$.
\fi  %todo ======================注释掉了========  
...
\subsection{估计}
多数情况下，必须通过数值方法求解 Lasso，如最小角回归法 (LARS)，坐标下降法等，只有附加一些严格的假设条件，才能获得 Lasso 的解析解。\footnote{%
   参见 \href{https://www.cnblogs.com/analysis101/p/14893843.html}{ Lasso 的解法}。
   不过，目前有很多高效快捷的算法。一个重要的性质是，当 $\lambda>0$ 时，即使 $p>n$，Lasso 估计量也具有唯一解。
   若变换 $\X$ 矩阵使所有解释变量都彼此正交，即 $\boldsymbol{X}^{\prime} \boldsymbol{X}=\boldsymbol{I}_{p}$，则 Lasso 存在解析解。此时只要给定调节参数 $\lambda$，只对 OLS 估计结果稍作调整即可得到 Lasso 估计。我们可以采用信息准则或交叉验证来确定最佳调节参数，留待第 \ref{sec:lasso-lambda} 小节详述。一般情况下，尤其是 $p>n$ 时，需采用数值方法求解。
}
...
0 & |\widehat{\beta}_{\text {OLS}}| \leq \lambda \\ \vspace{3pt}
\widehat{\beta}_{\text {OLS }}+\lambda & \widehat{\beta}_{\text {OLS }}<-\lambda
\end{array}\right.	
\end{equation}
...
\begin{equation}\label{eq:Lasso-ridge-01}
	...
	\end{gathered} 
\end{equation}
其中，$\tau>0$ 是收缩参数，约束效果呈现于图 \ref{Fig:Lasso-Ridge-regular-stata-b} 及图 \ref{fig:Lasso-ridge-compare-b}。用矩阵表示如下 (类比于 (\ref{eq:lasso-HTW2015-2-5}) 式)：\footnote{%
  注意，约束条件是 $\|\betaz\|_{2}^2$，而不是 $\|\betaz\|_{2}$，前者表示系数的平方和，而后者则表示系数的平方和再开根号 (参见第 \ref{sec:Lasso-norm} 小节)}
...
%\iffalse %todo ================注释-------begin----
%\def\subFigWide{5.5cm} %定义子图宽度
\def\subFigWide{0.8\textwidth}
\begin{figure}[htb!]  %[htbp]
\centering
\subfigure[Lasso]{
\includegraphics[width=\subFigWide]{Figs/Fig-Lasso-Lasso-figure-Hansen2021.png}
\label{fig:Lasso-ridge-compare-a}
}
\quad
\subfigure[岭回归]{
\includegraphics[width=\subFigWide]{Figs/Fig-Lasso-Ridge-figure-Hansen2021}
\label{fig:Lasso-ridge-compare-b}
}
\caption{Lasso与岭回归约束集对比}
\label{fig:Lasso-ridge-compare}
\end{figure}
%\fi      %todo ================注释-------over-----
...
\begin{figure}
	\centering
	\includegraphics[width=.8\textwidth]{Fig-Lasso-Ridge-figure-Hansen2021.png} 
	\caption{Ridge Regression Dual Minimization Solution} %caption是图片的标题
	\label{Fig-Lasso-Ridge-figure-Hansen2021} 
\end{figure}
\fi
...
\begin{tikzpicture}
    \begin{scope}[line width=0.5pt,font=\scriptsize]
        \draw [->] (-1.8,0) -- (4.5,0) node [below left]  {$\beta_1$};
        \draw [->] (0,-1.8) -- (0,3.5) node [below right] {$\beta_2$};
    \end{scope}
    % The circle is drawn with `(x,y) circle (radius)`
    % You can draw the outer border and fill the inner area differently.
    % Here I use gray, semitransparent filling to not cover the axes below the circle
    \path [draw=none,fill=gray!50,semitransparent] (0,0) circle (1.5);
    \draw[color=black] (0,0) circle (1.5);
    \draw[color=black,fill=black] (0,0) circle (0.06); %%% (0,0)圆点
    \path [draw=none,fill=black] (2.3, 1.45) circle (0.06); %%% 椭圆中的圆点
    \draw[color=black] (2.6,1.31) node [font=\tiny] {OLS}; 
    \draw[color=black,fill=black] (0.95, 1.18) circle (0.06); %%% 切点
    \draw[->,color=black!70] (0.8,0.7) node [below,font=\tiny,color=black] {Ridge} -- (0.95,1.1) ;  
    \draw[->,color=gray] (2.8,3.1) node [right,font=\tiny,color=black] {Ridge path} -- (1.9,1.52) ;  
    % Place the equation into the circle:
    \node [below right, darkgray, font=\scriptsize] at (+1.37,-0.02) {$\sqrt{\tau}$};
    \node [below right, darkgray, font=\scriptsize] at (+1.1,-1) {$\beta_1^2+\beta_2^2 \leq \tau$};
    \draw[rotate=-25] (1.4, 2.328) ellipse (2 and 1) ;
    \draw[rotate=-25,color=gray] (1.4, 2.328) ellipse (1.6 and 0.6) ;
    \draw[rotate=-25,color=gray] (1.4, 2.328) ellipse (1.0 and 0.4) ;
    \draw [densely dashed, path=curve] (0,0) .. controls (0.8, 1.8) and (2.3, 1.45) .. (2.3, 1.45);  %% 虚线曲线
\end{tikzpicture}
	\caption{Ridge Regression Dual Minimization Solution. \\ Source: Hansen (2021, Fig 29.1), 采用 Tikz 宏包重新绘制} %caption是图片的标题
	\label{Fig-Lasso-Ridge-figure3-Hansen2021} 
\end{figure}
\fi      %todo ================注释----岭回归图示---over-----

岭回归主要有两个用途。Hoerl and Kennard (1970) 最初提出该模型是为应对回归分析中的高度共线性问题。时至今日，则主要用于对高维数据和不可逆问题的正则化\index{正则化}。
...
在 Stata 中，可以用 \texttt{lasso2} (Ahrens et al., 2020), \texttt{ridgeregress}, \texttt{ridgereg} (提供了丰富的假设检验结果) 实现常规的岭回归分析。

IV 估计可以使用 \texttt{ridge2sls} 命令。 

面板数据模型可以用 \texttt{xtregfem}, \texttt{xtregmle} 命令。 
...
\begin{figure}
	\centering
	\includegraphics[width=1.2\textwidth]{Fig-Lasso-lp-norm-HTW2015-Fig2-6.png} 
	\caption{不同范数对应的约束集 \\ Source: HTW2015, Figure 2.6 Constraint regions $\sum_{j=1}^{p}\left|\beta_{j}\right|^{q} \leq 1$ for different values of $q$ For $q<1$, the constraint region is nonconvex.} %caption是图片的标题
	\label{Fig-Lasso-lp-norm-HTW2015-Fig2-6} 
\end{figure}



\subsection{Lasso 具有一致性的条件}

%todo Zou Hui (2006), p.1419 
We consider the lasso estimates, $\hat{\boldsymbol{\beta}}^{(n)}$,
$$
\hat{\boldsymbol{\beta}}^{(n)}=\underset{\beta}{\arg \min }\left\|\mathbf{y}-\sum_{j=1}^{p} \mathbf{x}_{j} \beta_{j}\right\|^{2}+\lambda_{n} \sum_{j=1}^{p}\left|\beta_{j}\right|,
$$
where $\lambda_{n}$ varies with $n$. Let $\mathcal{A}_{n}=\left\{j: \hat{\beta}_{j}^{(n)} \neq 0\right\}$. The lasso variable selection is consistent if and only if $\lim _{n} P\left(\mathcal{A}_{n}=\mathcal{A}\right)=1$.
...
\item \textbf{交叉验证 (CV)}：它属于数据驱动方法，更为关注模型的样本外预测能力，所需的假设条件较为宽松，在样本量较小的情况下尤为适用。%在 \texttt{cvlasso} 中实现。
...
它是 Stata 中 Lasso 系列命令的标配，也被外部命令 \texttt{rlasso} 所采用 (Arhens et al., 2020)。
\end{enumerate}
...
通常而言，以样本外预测为目标时，应选择以交叉验证为基础的方法，如下文介绍的 CV 和 adaptive-CV；以模型筛选为目标时，可以选择理论推演值 (plugin) 或信息准则 (如 BIC)，前者速度极快，后者在稀疏性较强的情况下表现稳定。\footnote{%
       这个结论主要根据笔者的模拟分析和实证经验而得，尚需查阅相关文献做进一步确认。}
...
Stata 中的 \texttt{lasso}, \texttt{lasso2} 等命令基本上覆盖了文献中的主流方法。
...
Stata 中的 Lasso 系列命令 (如 \texttt{lasso}, \texttt{elasticnet}, \texttt{poregress}, \texttt{xporegress}, \texttt{telasso}, \texttt{dsregress} 等) 都可以用 \texttt{selection(sel\_method)} 选项来指定 $\lambda^*$ 的选择方法，包括：\footnote{上述功能依赖于 Stata 16 以上版本，其中 \texttt{bic} 需要 Stata 17 以上版本。}
$$
\vspace{-3pt}
\begin{array}{ll}
\texttt{sel\_method } & \text { Description } \\
\hline \texttt{ cv} & \text { select $\lambda^*$ using CV; the default } \\
...
$\textcolor{blue}{\circ}$ \texttt{cv}：用 Lasso 做预测时，CV 是首选。

$\textcolor{blue}{\circ}$ \texttt{adaptive}：它是 $\mathrm{CV}$ 的变体，在每一步中它都先采用 CV 得到一个初步的 $\lambda^{*}$，进而使用自适应 Lasso 模型作进一步调整：放大重要系数，压缩不重要的系数。\texttt{adaptive-CV} 具有 CV 的样本外推能力 (因为在选择 $\lambda$ 的过程中，评价标准就是样本外 MSE 的大小)，同时根据变量的相对重要性做出二次调整。因此，它选出的模型比基于 CV 得到的模型更精简，但预测能力却不会有太多折损。我们将在第 \ref{sec:adaptive-lasso} 介绍 Adaptive Lasso。

$\textcolor{blue}{\circ}$ \texttt{plugin}：当研究目的不是样本外预测，而是模型筛选时，\texttt{plugin} 的应用最广。因为其 $\lambda^*$ 是基于理论推演得到的，省去了搜索过程。

$\textcolor{blue}{\circ}$ \texttt{bic}：是 Stata 17 新增的功能，应用较少。因为它和 CV 一样，都很耗时；但样本外预测能力又不如 CV。

Stata 手册对 Lasso 的模型筛选过程进行了详细介绍，参见 

$\textcolor{red}{\circ}$ Lasso 整体介绍：\href{https://www.stata.com/manuals/lassolasso.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso}}

$\textcolor{red}{\circ}$ Lasso 模型筛选：\href{https://www.stata.com/manuals/lassolasso.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso model selection}}

此外，实现各类 Lasso 模型的 Stata外部命令，如 \texttt{lasso2}, \texttt{rlasso}, \texttt{elasticregress}, \texttt{pdslasso}, \texttt{ivlasso} 等，也都内置了 $\lambda^*$ 的选择方法。 

\vspace{5pt}
综上所述，确定 $\lambda^{*}$ 的基本\textbf{经验规则}是：
\vspace{5pt}

(1) 若研究目的是模型筛选，则 \texttt{plugin} 是很好的选择。

(2) 若研究目的是预测，则可以在测试阶段 (此时对结果的准确度要求不高) 使用 \texttt{plugin}，以节省计算时间。随后，可以采用 \texttt{adaptive}-CV 和普通 \texttt{cv} 做更严格的测试。

(3) 当样本较小时 (比如 $n<500$)，\texttt{cv} 和 \texttt{adaptive}-CV 表现都很不稳定，甚至无法搜索到合适的 $\lambda$ 值，此时可以使用 \texttt{plugin} 和信息准则。
...
有关 Stata 中 $\lambda^*$ 的计算流程，参见 \href{https://www.stata.com/manuals/lassolasso.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso model selection}}, pp.4。帮助文件为 \texttt{help lasso\#\#selmethod}。
...
\iffalse %todo ================注释-------begin----
%todo Ahrens, 2020
Information criteria are closely related to regularization methods. The classical AIC (Akaike 1974) is defined as $-2 \times \log$ likelihood $+2 p$. Thus, the AIC can be interpreted as penalized likelihood, which imposes a penalty on the number of predictors included in the model. However, this form of penalization, referred to as the $\ell_{0}$ penalty, has an important practical disadvantage. To find the model with the lowest AIC, we need to estimate all different model specifications. In practice, it is often not feasible to consider the full model space. For example, with only 20 predictors, there are more than $1,000,000$ different models.

The advantage of regularized regression is that it provides a data-driven method for reducing model selection to a one-dimensional problem (or two-dimensional problem in the case of the elastic net) where we need to select $\lambda$ (and $\alpha$ ). Theoretical properties of information criteria are well understood, and they are easy to compute once coefficient estimates are obtained. Thus, it seems natural to use the strengths of information criteria as model-selection procedures to select the penalization level.

Information criteria can be categorized based on two central properties: loss efficiency and model-selection consistency. A model-selection procedure is loss efficient if it yields the smallest averaged squared error attainable by all candidate models. Model-selection consistency requires that the true model be selected with probability approaching 1 as $n \rightarrow \infty$. Accordingly, which information criteria is appropriate in a given setting also depends on whether the aim of analysis is prediction or identification of the true model. 
\fi      %todo ================注释-------over-----

简要回顾一下 AIC, BIC, HQIC 等信息准则的计算公式：\footnote{%
   关于 AIC，BIC 以及 Best subset 估计量的推导过程和详细来由，参见 Fan et al. (\href{https://www.taylorfrancis.com/books/mono/10.1201/9780429096280/statistical-foundations-data-science-jianqing-fan-runze-li-cun-hui-zhang-hui-zou}{2020}), Section 3.1 Classical Variable Selection Criteria。%
}

%\vspace{6pt}
$\textcolor{blue}{\circ}$ AIC (Akaike Information Criterion)：赤池信息准则
$$\mathrm{AIC} = -2 \ln(L) + 2 k$$

%\vspace{6pt}
$\textcolor{blue}{\circ}$ BIC (Bayesian Information Criterion)：贝叶斯信息准则
$$\mathrm{BIC}=-2 \ln(L) + \ln(n)k$$

%\vspace{6pt}
$\textcolor{blue}{\circ}$ HQIC (Quinn Criterion)：汉南-奎因信息准则
...
\end{equation}
\begin{equation}\label{eq:lasso-bic}
...
$\mathrm{AlC}$ 和 $\mathrm{BIC}$ 都不太适合“大 $p$ 小$N$” 情形，\footnote{%
  这句话的准确意思是 $p \gg n$，是个相对概念。因此，“大 $p$ 小$N$” 并不意味着 $N$ 一定很小。}
...
交叉验证的基本思想是把原始数据(dataset)进行分组，一部分做为训练集(train set)，另一部分做为验证集 (validation set or test set)，首先用训练集对模型进行估计，再利用验证集来测试训练得到的模型(model)，以此评估模型的表现。\footnote{%
  CV 主要用在数据不是很充足的情况下。如果样本较大 (如超过 10000 笔观察值)，可以直接将其随机分成三份：A. 训练集 (Training Set)；B. 验证集 (Validation Set)；C. 测试集 (Test Set)。用 A 来训练 (估计) 模型；用 B 来评估模型的预测能力、进行模型选择和变量筛选；进而把得到的模型用于 C，最终决定使用哪个模型以及对应参数。}

%todo https://www.stata.com/manuals/lassolassofitting.pdf#lassolassofitting

根据切分的方法不同，交叉验证分为下面三种：\footnote{%
   多数情况下，交叉验证都是作为特定估计量的内嵌程序而执行的。若需单独做交叉验证，或希望单独执行以切身理解计算过程，可以使用 \texttt{cv\_kfold} 命令，其他命令还包括：\texttt{crossfold}, \texttt{crossvalidate},  \texttt{cv\_regress}, \texttt{kfoldclass} 等。
}
...
\begin{figure}
	\centering
	\includegraphics[width=.8\textwidth]{Fig-Lasso-CV-rolling-03.png} 
	\caption{K 折交叉验证示意图} %caption是图片的标题
	\label{Fig-Lasso-CV01} 
\end{figure}

Fig. 2 Rolling $h$-step ahead cross-validation with expanding training window. ' $T$ ' and ' $V$ ' denote that the observation is included in the training and validation sample, respectively. A dot $(\because$, ') indicates that an observation is excluded from both training and validation data


%todo Cerulli-2021-Machine Learning using Stata-Python.pdf
Cross-validation is the workhorse of the test-error estimation in machine learning. The idea is to randomly divide the initial dataset into $K$ equal-sized portions called folds. This procedure suggests to leave out fold $k$ and fit the model to the other $(K-1)$ folds (wholly combined) to then obtain predictions for the left-out $k$-th fold. This is done in turn for each fold $k=1,2, \ldots, K$, and then the results are combined by averaging the $(K-1)$ estimates of the error. The cross-validation procedure can be carried out as follows:
- Split randomly the initial dataset into $K$ folds denoted as $G_{1}, G_{2}, \ldots, G_{K}$, where $G_{k}$ refers to part $k$. Assume that there are $n_{k}$ observations in fold $k$. If $N$ is a multiple of $K$, then $n_{k}=n / K$.

- For each fold $k=1,2, \ldots, K$, compute:
$$
\mathrm{MSE}_{k}= 
\frac{1}{n_k}\sum_{i \in C_{k}}\left(y_{i}-\widehat{y_{i}}\right)^{2}
$$
where $\widehat{y_{i}}$ is the fit for observation $i$, obtained from the dataset with fold $k$ removed.

- Compute:
$$
\mathrm{CV}_{K}=\sum_{i=1}^{K} \frac{n_{k}}{n} \mathrm{MSE}_{k}
$$

that is the average of all the out-of-sample $M S E$ obtained fold-by-fold.

Observe that, by setting $K=n$, we obtain the $n$-fold or leave-one out cross-validation $(\mathrm{LOOCV}) .$ Also, as $C V_{K}$ is an estimation the true test-error, estimating its standard error can be useful to provide a confidence interval and thus a measure of test-accuracy's uncertainty.

For ML classification purposes, finally, the cross-validation procedure follows the same line, except for considering the MCE in place of the MSE.
\fi      %todo ================注释-------over-----

\vspace{10pt}
\textbf{几点说明：}

其一，执行 $K$ 折交叉验证需要将观察值随机分成 $K$ 组。因此，当样本数较少时，CV 选出的 $\lambda^*$ 具有较大的不确定性。此时，可以通过 \texttt{set seed \#} 设定种子值，以保证实证结果的可复制性。当然，需要确保实证结果不会对种子值的选取太敏感。一般来说，设定较大的 $K$ 值有助于降低这种敏感性，但当样本数较小时，这其实也不太可行。Stata 的默认值是 $K=10$，这也是多数软件和文献的基本设定。实证分析中可以适当修改之以测试结果的稳健性。另一种方法是使用“重复 CV 法”，即把上述 CV 过程重复做多次，取平均，以降低随机分组和 $K$ 值选取带来的影响。

% one se 规则

其二，面板数据通常具有较强的组内相关性，需要以组别为单位进行 CV，而对于时间序列，问题可能更为复杂，需要使用滚动 CV 或分块 CV 等方法，参见 Ahrens et al. (\href{http://sci-hub.ren/10.1177/1536867X20909697}{2020}, Section 4)。 
...
有关 CV 的更深入的介绍，参见 Ahrens et al. (\href{http://sci-hub.ren/10.1177/1536867X20909697}{2020}, Section 4)，以及 Hansen (\href{https://www.ssc.wisc.edu/~bhansen/econometrics/Econometrics.pdf}{2021}, Section 28.9)。
...
若主要目标是变量筛选 (如后文介绍的 Lasso-IV，或 Double Selection)，采用由理论上推导出的经验值 $\lambda_{\text{plugin}}^*$ 既省时又省力，因为这无需执行任何搜索程序。尤其是当 CV 无法使用或很不稳定时，$\lambda_{\text{plugin}}^*$ 是很好的替代。\footnote{%
   例如，当样本较小时，基于 CV 的方法会对随机分组非常敏感，且经常因为无法找出合适的 $\lambda$ 而导致程序中止。此时，Stata 会显示错误提示信息：{\\ \textcolor{red}{Note: No lambda selected. \texttt{lassoselect} can be used to select lambda. \\
   Note: Minimum of CV function not found; lambda selected based on \texttt{stop()} stopping criterion.}}}
此外，相对于 CV 和 BIC，\texttt{plugin} 选出的变量都更少，因此模型更为精简。

%此时，plugin 就会非常奏效，因为它给出的 $\lambda^*$ 是从理论上直接推到出来的，。Belloni et al. (2014a, JEP) 文末提供的 \texttt{lassoShooting.ado} 程序 (\href{https://academic.oup.com/restud/article/81/2/608/1523757}{Stata-codes}) 便采用了基于 plugin 的 $\lambda^*$，\footnote{%
%  该程序的功能可以用 Stata 16 以上版本中的 \texttt{lasso linear, select(plugin)} 命令完全替代，不必另外下载。}
%计算公式见 (\ref{eq:lasso-plugin-01}) 和 (\ref{eq:lasso-plugin-02}) 式。

$\lambda_{\text{plugin}}^*$ 的推导和证明过程都比较复杂，这里仅列出一些 Stata 手册 \href{https://www.stata.com/manuals/lassolasso.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso, pp.27-30}} 
...
%Stata 中的 Lasso 系列命令基本上都支持 \texttt{selection(plugin)} 选项，具体设定方式及含义如下：

在 Stata 中，若采用 \texttt{lasso linear}，则有两个 plugin 估计量可供选择：
\begin{itemize}
\item \texttt{selection(plugin, homoskedastic)} 选项：
...
\item \texttt{selection(plugin, heteroskedastic)} 选项：
...
$\textcolor{blue}{\circ}$ $c=1.1$ (Belloni and Chernozhukov (2011) 建议的数值)；

$\textcolor{blue}{\circ}$ $N$ 是样本量；

$\textcolor{blue}{\circ}$ $\gamma=0.1 / \ln [\max \{p, N\}]$ 为系数为零时不移除该变量的概率；

$\textcolor{blue}{\circ}$ $p$ 是模型中备选变量的个数。
...
\iffalse %todo ================注释-------begin----
%todo 参见 help lasso2 中的说明
%todo Stata 手册


Plugin selection
CV selects model $\left(\lambda^{*}\right)$ such that $f(\lambda)$ is minimized. Adaptive is a variation on CV. It selects a final $\operatorname{model}\left(\lambda^{*}\right)$ that minimizes a more restricted $f(\lambda)$.

Plugins-selection (plugin) - are a whole different thing. Parameter $\lambda$ still plays a role, but $f(\cdot)$ does not. Instead, the $\lambda^{*}$ that determines $\operatorname{model}\left(\lambda^{*}\right)$ is produced by direct calculation using the plugin function, $\lambda^{*}=g(\cdot)$. The function returns the smallest value of $\lambda$ that is large enough to dominate the estimation error in the coefficients.
...
$\mathrm{CV}$ is implemented for lasso, elasticnet, and sqrtlasso. Adaptive lasso is implemented in lasso. Plugin estimators are implemented for lasso and for sqrtlasso. BIC is implemented for lasso, elasticnet, and sqrtlasso.
...
Zou and Hastie (2005) 提出的弹性网 (Elastic Net, 简称 ENet) 有助于克服上述问题，\footnote{%
  另一个可以解决此类问题的方法是“群组 Lasso” (Group Lasso)，其基本思想是，若预先明确知道哪些变量存在较强的相关性，则可以在设定惩罚项时将他们放在同一组，用 $\ell_{2}$ 范数约束它们，而组别之间则使用 $\ell_{1}$ 范数来约束。相比之下，
  ENet 则适用于无法预先依据相关性对变量分组的情形。}%
...
\begin{figure}
	...
	\caption{Source: Hastie, Tibshirani and Wainwright (\href{https://web.stanford.edu/~hastie/StatLearnSparsity_files/SLS.pdf}{2015}). Statistical Learning with Sparsity: The Lasso and
	Generalizations. Boca Raton, FL: CRC Press. \href{https://web.stanford.edu/~hastie/StatLearnSparsity/}{Link}. \href{https://web.stanford.edu/~hastie/StatLearnSparsity_files/SLS.pdf}{PDF}HTW2015, Figure 4.1 Six variables, highly correlated in groups of three. The lasso estimates $(\alpha=1)$, as shown in the left panel, exhibit somewhat erratic behavior as the regularization parameter $\lambda$ is varied. In the right panel, the elastic net with $(\alpha=0.3)$ includes all the variables, and the correlated groups are pulled together.} %caption是图片的标题
	\label{\FigName} 
...
$$
\hat{\beta}^{\text {ridge }}=\underset{\beta}{\arg \min }\left\{\sum_{i=1}^{N}\left(y_{i}-\beta_{0}-\sum_{j=1}^{p} x_{i j} \beta_{j}\right)^{2}+\lambda \sum_{j=1}^{p}\left|\beta_{j}\right|+\lambda \sum_{j=1}^{p} \beta_{j}^{2}\right\}
$$
若令 $\lambda=\lambda_{1}+\lambda_{2}, \alpha=\frac{\lambda}{\lambda_{1}+\lambda_{2}}$ 则
$$
\hat{\beta}^{\text {ridge }}=\underset{\beta}{\arg \min }\left\{\sum_{i=1}^{N}\left(y_{i}-\beta_{0}-\sum_{j=1}^{p} x_{i j} \beta_{j}\right)^{2}+\lambda \sum_{j=1}^{p}\left(\alpha\left|\beta_{j}\right|+(1-\alpha) \beta_{j}^{2}\right)\right\}
$$
由此可知，弹性网的罚函数 $\lambda \sum_{j=1}^{p}\left(\alpha\left|\beta_{j}\right|+(1-\alpha) \beta_{j}^{2}\right)$ 恰好为岭回归罚函数和Lasso罚函数的一个凸线性组合. 当 $\alpha=0$ 时，弹性网 回归即为岭回归; 当 $\alpha=1$ 时，弹性网回归即为Lasso回归. 因此，弹性网 回归兼有Lasso回归和岭回归的优点，既能达到变量选择的 目的，又具有很好的群组效应.

\fi      %todo ================注释-------over-----

%\subsubsection{Irrepresentable Condition (IRC)}
%
%Zhao and (\href{https://www.jmlr.org/papers/volume7/zhao06a/zhao06a.pdf}{2006}) 
...
这种设定可以让扰动项的标准差成为一个常数，\footnote{%
  类似于对随机变量进行标准化，使其方差为 1，从而可以将其从目标函数中拿掉。}
更易于求解调节参数 $\lambda$ 的理论最优值 $\lambda^*$。\footnote{%
      即第 \ref{sec:lasso-plugin} 小节的 plugin 估计值。}
该值可以为双重 Lasso (double-selection)、偏出 Lasso (partialing-out methods) 等需要多次 Lasso 估计的方法提供 $\lambda$ 的初始值，提高求解效率。因此，sqrt-Lasso 通常是配合其他方法来使用，很少单独使用。比如，对于 \ref{sec:lasso-DSL} 小节介绍的 Double Selection Lasso，可以在 \texttt{dsregress} 命令中附加 \texttt{sqrtlasso} 选项，用 sqrt-Lasso 进行变量筛选。

若需独立估计 sqrt-Lasso 模型，可以用 \texttt{sqrtlasso} 命令，详见 \href{https://www.stata.com/manuals/lassosqrtlasso.pdf}{\bfseries{[\MakeUppercase{lasso}] sqrtlasso}}。
...
\iffalse %todo ================注释-------begin----
%todo Ahrens2020-SJ, ft 3
It can be shown that the square-root lasso is equivalent to the scaled lasso, which solves the minimization problem
$$
\min _{\beta, \sigma} \frac{1}{2 \sigma} \sum_{i=1}^{n}\left(y_{i}-\boldsymbol{x}_{i}^{\prime} \boldsymbol{\beta}\right)^{2}+\frac{n \sigma}{2}+\lambda \sum_{j=1}^{p} \psi_{j}\left|\beta_{j}\right|
$$
...
简言之，ALasso 主要用于超稀疏情形，可以在更一般化的条件下确保模型筛选的一致性。这种“自适应”的想法也被用于弹性网络和岭回归 (Zou and Zhang 2009)。自适应 Lasso 被广泛应用于模型筛选，比如确定 $\lambda^*$，它是 Stata 中用于确定 $\lambda^*$ 的四种方法之一。详见 Stata 手册 \href{https://www.stata.com/manuals/lassolassofitting.pdf#lassolassofitting}{Lasso fitting}, pp.11。%在 Stata 中，ALasso 是选择 $\lambda^*$ 的一个重要手段。
...
Stata 中可以用 \texttt{lasso, selection(adaptive, steps(\#))} 命令估计 ALasso 模型，\texttt{\#} 表示 Lasso 的次数，数值越大产生的模型越稀疏。对于 Stata 15 及以下版本的用户，可以使用外部命令 \texttt{lasso2} (Ahrens et al., 2020, \href{http://sci-hub.ren/10.1177/1536867X20909697}{PDF})。

%todo https://www.stata.com/manuals/lassolassofitting.pdf#lassolassofitting
\iffalse %todo ================注释-------begin----
steps (#) specifies that adaptive lasso with # lassos be done. By default, #=2. That is, two lassos are run. After the first lasso estimation, terms with nonzero coefficients $\beta_{i}$ are given penalty weights equal to $1 /\left|\beta_{i}\right|$, terms with zero coefficients are omitted, and a second lasso is estimated. Terms with small coefficients are given large weights, making it more likely that small coefficients become zero in the second lasso. Setting $\#>2$ can produce more parsimonious models. See Methods and formulas.  %todo lasso 手册，pp.6
\fi      %todo ================注释-------over-----
...
Lasso 既可以用于预测，也可以用于变量筛选，这是其在因果推断中发挥着重要作用，比如估计反事实结果，克服冗余工具变量问题等。\footnote{%
    在 Stata 中输入 \texttt{help lasso}，帮助文件顶部显示的信息为：\texttt{lasso -- Lasso for prediction and model selection}。}
...
%Stata 专门提供了 \texttt{telasso} 命令来实现各类基于 Lasso 的处理效应模型。
...
第二步，用 $Y$ 对 $X_{S}$ 执行 OLS 估计，得到系数估计值，\footnote{%
   Belloni and Chernozhukov (2013) 提供了 Post-Lasso 与 Lasso 估计量具有相同收敛率的前提条件。}
...
需要特别强调的是，Post-Lasso 的价值主要在于其背后的估计思想，若直接应用 Post-Lasso 反而会有很多局限 (详见 \href{https://www.stata.com/manuals/lassoLasso intro.pdf}{\bfseries{[\MakeUppercase{lasso}] Lasso intro}}, pp.7)：
...
或许是基于上述考虑，Stata 并未提供用于单独执行 Post-Lasso 的命令，而是在执行完 \texttt{lasso} 后，使用 \texttt{lassocoef, display(coef, postselection)} 来呈现其估计系数，详见 \texttt{help lassocoef}。
若使用外部命令 \texttt{rlasso} 或 \texttt{lasso2}，则只需附加 \texttt{ols} 选项即可。 
...
$$
In later work (see Section 29.22) the authors describe $\widehat{\beta}_{\text {Lasso, } B}$ as a "sample split" and $\widehat{\beta}_{\text {Lasso-SsIV }}$ as a "cross-fit" estimator.
...
假设%采用 Group-Lasso，
在控制核心变量 $D$ 的情况下，\footnote{%
   也就是说，变量 $D$ 不参与 Lasso 筛选。可以用 \texttt{lasso, offset(D)} 命令来实现。}
...
为此，Belloni, Chernozhukov, and Hansen (\href{http://sci-hub.ren/10.1093/restud/rdt044}{2014b}, RES) 提出了 Double Selection Lasso (简称 DS-Lasso)。基本思想是：分别用 $Y$ 和 $D$ 对 $X$ 执行 Lasso 回归，并将两次筛选出的变量的并集作为最终的控制变量集 $X_{DS}$。最终，用 $Y$ 对 $D$ 和 $X_{DS}$ 执行 OLS 回归即可。

Belloni et al. (\href{http://sci-hub.ren/10.1093/restud/rdt044}{2014b}) 的理论分析表明，若 $Y$, $D$ 与 $X$ 之间都满足近似稀疏性假设 (模型可以用数量有限的一组变量来很好地近似)，则 DS-Lasso 估计量及其 $t$ 值都渐进地服从正态分布。因此，常规的统计推断方法仍然适用。注意，这些结论都以 $N \to \infty$ 为前提，在有限样本下仍然可能存在较大的偏差。%同时，上述结果也近似稀疏性假设 
...
下面摘取 Stata 手册 \href{https://www.stata.com/manuals/lassodsregress.pdf}{\bfseries{[\MakeUppercase{lasso}] dsregress}} (p.6) 中的表述来说明 DS-Lasso 的实现过程。
...
DS-Lasso 的实现过程如下：
\begin{enumerate}
\item 用 $y$ 对 $\mathbf{x}$ 执行线性 Lasso 估计 (Stata命令为：\texttt{lasso linear y x*}), 将选出的变量集记为 $\widetilde{\mathbf{x}}_{y}$。
...
\item 用 $y$ 对 $\mathbf{d}$ 和 $\widehat{\mathbf{x}}$ 执行线性回归，对应的系数估计估计值记为 $\widehat{\boldsymbol{\alpha}}$ 和 $\widehat{\boldsymbol{\beta}}$。Stata 中默认计算稳健性标准误，即 \texttt{reg y d x\_hat, vce(robust)}。
\end{enumerate}
补充两点说明：

其一，第 1-2 步中的 Lasso 估计可以用第 \ref{sec:lasso-lambda} 小节介绍的 \texttt{cv}, \texttt{plugin}, \texttt{adaptive} 等方法确定调节参数 $\lambda^*$。若不做设定，则 Stata 默认采用 plugin 方法获选择 $\lambda^*$，即 (\ref{eq:lasso-plugin-02}) 式中的 $\lambda_{\text {heteroskedastic}}$。

其二，第 3 步中之所以「取并集」，是为了避免遗漏变量偏误。换言之，无论是对 $y$ 还是对 $d_j$ 有影响的变量都应保留，相当于做了个「双保险」。这是 DS-Lasso 用以克服 Post-Lasso 局限的主要手段。
 

\iffalse %todo ================注释-------begin----
%todo Hansen2021  随后补上这部分理论内容
Post-estimation inference is difficult with most machine learning estimators. For example, consider the post-Lasso estimator (least squares applied to the regressors selected by the Lasso). This is a postmodel-selection (PMS) estimator, as discussed in Sections $28.16$ and 28.17. As shown in Section 28.17,

the coverage probability of standard confidence intervals applied to PMS estimators can be far from the nominal level. Belloni, Chernozhukov, and Hansen (2014b) proposed an alternative estimation and inference method which achives better coverage rates.
...
\item Let $\tilde{X}=X_{1} \cup X_{2}$ be the union of the variables in $X_{1}$ and $X_{2}$.
...
Stata 中可以用 \texttt{dsregress} 命令得到 DS-Lasso 估计量，详见 \href{https://www.stata.com/manuals/lassodsregress.pdf}{\bfseries{[\MakeUppercase{lasso}] dsregress}}。外部命令为 \texttt{pdslasso}。
...
\subsubsection{注意事项}
%todo telasso 手册, p.7
Model selection, however, does not come for free. If researchers use model selection but conduct inference ignoring the fact that they did model selection, the inference results are possibly wrong. This is because model selection techniques make mistakes. Making inferences without considering the variability in model selection is a dangerous practice (see Leeb and P?tscher [2005] and Leeb and P?tscher [2006]). Instead, the inference should be robust to model selection mistakes.
...
\texttt{telasso} implements estimators in Chernozhukov et al. (2018) for the ATEs, ATETs, and POMs from observational data.

\vspace{5pt}
The \texttt{telasso} estimator uses a five-step approach to estimating ATE:
\begin{enumerate}
\item It uses lasso techniques to select variables in the outcome model for each treatment level.
\item Based on the selected variables in step 1, it fits separate regression models of the outcome for each treatment level and obtains the treatment-specific predicted outcomes for each subject.
\item It uses lasso techniques to select variables in the treatment model.
\item Based on the selected variables in step 3, it estimates the parameters of the treatment model and computes inverse-probability weights.
\item It computes the weighted means of the treatment-specific predicted outcomes, where the weights are the inverse-probability weights computed in step 4. The contrasts of these weighted averages provide the estimates of the ATE.
\end{enumerate}
...
\texttt{telasso} also implements the double machine learning estimator in Chernozhukov et al. (2018) for ATEs, ATETs, and POMs. Double machine learning relaxes the sparsity assumption needed for lasso methods. The sparsity assumption implies that lasso can only have good properties if it selects a few variables from a potentially large number of candidates. Double machine learning allows \texttt{telasso} to select more variables in the models and still be valid.
\texttt{telasso} allows the outcome variable to be modeled using a linear, logistic, probit, or Poisson model. For a linear outcome model, square-root lasso can be used instead of lasso in step 1.
...
%\href{https://www.stata.com/manuals/lassoporegress.pdf}{\bfseries{[\MakeUppercase{lasso}] poregress}} 
...
Stata 手册将该模型称为「偏出 Lasso (Partialing-out Lasso)」，\footnote{%
   参见 \href{https://www.stata.com/manuals/lassoLasso_intro.pdf}{\bfseries{[\MakeUppercase{lasso}] Lasso\_intro}}，以及 \href{https://www.stata.com/manuals/lassoporegress.pdf}{\bfseries{[\MakeUppercase{lasso}] poregress}}。}。
一方面是因为该方法的核心思想源于著名的 Frisch-Waugh-Lovell 定理 (Frisch and  Waugh, 1933; Lovell, 1963; Yamada, \href{http://dx.doi.org/10.1080/03610926.2016.1252403}{2017}))，\footnote{%
   参见 \href{https://www.lianxh.cn/news/e346db1a68211.html}{图示线性回归系数：Frisch-Waugh定理与部分回归图}。}
...
Stata 中的处理过程与此相似，参见 \href{https://www.stata.com/manuals/lassoporegress.pdf}{\bfseries{[\MakeUppercase{lasso}] poregress}}, p.7-8。外部命令为 \texttt{pdslasso}。

上述方法也可以扩展到模型中包含内生变量的情形，此时便是“偏出 IV-Lasso”估计。参见第 \ref{sec:lasso-IV} 小节，Chernozhukov, Hansen, and Spindler (2015a, b)，以及 \href{https://www.stata.com/manuals/lassopoivregress.pdf}{\bfseries{[\MakeUppercase{lasso}] poivregress}}, pp. 7。Stata 命令为 \texttt{poivregress}。 


\iffalse %todo ================注释-------begin----

我们首先变换结构方程（29.22）以消除高维分量。 取以 $X$ 为条件的 (29.22) 的期望值，并从每一边减去。 这导致方程 
...
\item  Estimate (29.24) by Lasso or post-Lasso with Lasso parameter $\lambda_{2}$. Let $\widehat{\eta}$ be the coefficient estimator and $\hat{U}_{i}=Y_{i}-X_{i}^{\prime} \hat{\eta}$ the residual.
...
The SPOLR method extends naturally to nonlinear models for the outcome and has two sources. It is implied by Belloni, Chernozhukov, and Kato (2015, algorithm 1), which is for a median regression of $y$ on $\mathrm{x}$. Replacing median regression with mean regression in their step (i) and replacing the median moment condition with the mean moment condition in step (iii) produces the SPOLR algorithm detailed below. This algorithm is also implied by Belloni, Chernozhukov, and Wei $(2016$, table 1 and sec. 2.1) for a linear model.
...
\subsubsection{POLR algorithm}
\begin{enumerate}
\item  For $j=1, \ldots, J$, perform a linear lasso of $d_{j}$ on $\mathbf{x}$, and denote the selected controls by $\widetilde{\mathbf{x}}_{j}$. Each of these lassos can choose the lasso penalty parameter $\left(\lambda_{j}^{*}\right)$ using the plugin estimator, adaptive lasso, or CV. The heteroskedastic plugin estimator for the linear lasso is the default.
\item  For $j=1, \ldots, J$, fit a linear regression of $d_{j}$ on $\widetilde{\mathbf{x}}_{j}$, denote the estimated coefficients by $\widehat{\gamma}_{j}$, and define the partial-covariate variable $z_{j}=d_{j}-\widetilde{\mathbf{x}}_{j} \widehat{\gamma}_{j}$, with its $i$ th observation denoted by $z_{j, i}$
Collect the $J$ partial covariates for the $i$ th observation into the vector $\mathbf{z}_{i}=\left(z_{1, i}, \ldots, z_{J, i}\right)$.
\item  Perform a linear lasso of $y$ on $\mathbf{x}$ to select covariates $\widetilde{\mathbf{x}}_{y}$.
This lasso can choose the lasso penalty parameter $\left(\lambda^{*}\right)$ using the plugin estimator, adaptive lasso, or CV. The heteroskedastic plugin estimator for the linear lasso is the default.
\item  Fit a linear regression of $y$ on $\widetilde{\mathbf{x}}_{y}$, denote the estimated coefficients by $\widetilde{\boldsymbol{\beta}}_{y}$, and define the partial outcome for the $i$ th observation by $\widetilde{y}_{i}=y_{i}-\widetilde{\mathbf{x}}_{y, i} \widetilde{\boldsymbol{\beta}}_{y}$
\item  Compute $\widehat{\alpha}$ by solving the following $J$ sample-moment equations.
$$
\frac{1}{n} \sum_{i=1}^{n}\left(\widetilde{y}_{i}-\mathbf{z}_{i} \boldsymbol{\alpha}^{\prime}\right) \mathbf{z}_{i}^{\prime}=\mathbf{0}
$$
\end{enumerate}
The VCE is estimated by the robust estimator for method of moments.


\subsubsection{SPOLR algorithm}
\begin{enumerate}
\item  For $j=1, \ldots, J$, perform a linear lasso of $d_{j}$ on $\mathbf{x}$, and denote the selected controls by $\widetilde{\mathbf{x}}_{j}$. Each of these lassos can choose the lasso penalty parameter $\left(\lambda_{j}^{*}\right)$ using the plugin estimator, adaptive lasso, or CV. The heteroskedastic plugin estimator for the linear lasso is the default.
\item  For $j=1, \ldots, J$, fit a linear regression of $d_{j}$ on $\widetilde{\mathbf{x}}_{j}$, denote the estimated coefficients by $\widehat{\gamma}_{j}$, and define the partial-covariate variable $z_{j}=d_{j}-\widetilde{\mathbf{x}}_{j} \widehat{\gamma}_{j}$, with its $i$ th observation denoted by $z_{j, i}$
Collect the $J$ partial covariates for the $i$ th observation into the vector $\mathbf{z}_{i}=\left(z_{1, i}, \ldots, z_{J, i}\right)$.
\item  Perform a linear lasso of $y$ on $\mathbf{d}$ and $\mathbf{x}$ to select covariates $\check{\mathbf{x}}_{y}$.
This lasso can choose the lasso penalty parameter $\left(\lambda^{*}\right)$ using the plugin estimator, adaptive lasso, or CV. The heteroskedastic plugin estimator for the linear lasso is the default.
\item  Fit a linear regression of $y$ on $\mathbf{d}$ and $\check{\mathbf{x}}_{y}$, let $\breve{\boldsymbol{\beta}}$ be the estimated coefficients on $\check{\mathbf{x}}_{y}$, and define the partial outcome for the $i$ th observation by $\check{y}_{i}=y_{i}-\check{\mathbf{x}}_{y, i} \breve{\boldsymbol{\beta}}^{\prime}$
\item  Compute $\widehat{\boldsymbol{\alpha}}$ by solving the following $J$ sample-moment equations.
$$
\frac{1}{n} \sum_{i=1}^{n}\left(\check{y}_{i}-\mathbf{d}_{i} \boldsymbol{\alpha}^{\prime}\right) \mathbf{z}_{i}^{\prime}=\mathbf{0}
$$
\end{enumerate}
The VCE is estimated by the robust estimator for method of moments.
...
\iffalse %todo ================注释-------begin----
\section{Lasso IV}\label{sec:lassoIV}
\subsection{为何要用 Lasso IV？}
%todo Chen2012, Appendix G lasso

Source: Judges: Chen, Daniel L., and Susan Yeh. 2012. "Growth under the Shadow of Expropriation? The Economic Impacts of Eminent Domain." \href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2977074}{Link}

Some econometricians recommend larger first stage F-statistics to ensure that the first stage is sufficiently strong, such as F stat $=25$ or 50 to allow for heteroskedasticity and serial autocorrelation (?). LASSO presents a way to optimally extract information from the combinatorially many instrumental variables available for use. Using LASSO (least absolute shrinkage and selection operator) in the first stage presents several advantages relative to using OLS.

While OLS has low bias, it also has two disadvantages. 

First, OLS lacks sparseness: large subsets of covariates are deemed important, resulting in too many instruments, which makes 2SLS susceptible to a weak instruments problem. 

Second, OLS lacks continuity: changing the data a bit results in different subsets of covariates deemed important. 

LASSO is a sparse model, which solves both of these problems. Formally, LASSO modifies OLS by minimizing the sum of squares subject to the sum of the absolute value of the coefficients being less than a constant. The nature of this constraint tends to set some coefficients to exactly 0 and hence reduces model complexity. Intuitively, LASSO gives interpretable models by imposing a data penalty for having too many covariates. 

In addition, LASSO ensures stability in instrument selection, making it an effective tool in selecting optimal instrumentals from a large number of valid instruments. Because it selects optimal instruments, LASSO enhances statistical precision when using the random assignment of judges. Belloni et al. (2011) show that the increased uncertainty due to selecting among many instruments does not show up to first order.

%todo https://www.sole-jole.org/assets/docs/13463.pdf
To construct our potential LASSO instruments, we use 30 biographical characteristics and their interactions at the judge level and panel level (for example, for the combination of "black and Democrat," we examine the number of black Democratic appointees per seat for the judge level interactions and examine the number of Democrat appointees per seat multiplied by the number of black judges per seat for panel level interactions) yielding a total of 900 possible instruments. The instruments chosen by the LASSO procedure are listed in Appendix Table 1.7. For example, at the circuit-year level, the LASSO procedure selected Democrat prior assistant U.S. Attorneys for regulatory takings. Note that the number of observations per circuit-year or the number of years of data varies across outcomes. The F statistic with LASSO instruments is 38 , representing a $100 \%$ improvement over the non-LASSO first stage F-statistics displayed in Column 5 of Panel $\mathrm{C}$ in Appendix Tables $1.4$ and $1.5$. We do not use a many weak instruments method since our basic instrument is already quite strong.

On a separate note, the use of the LASSO instruments provides a check of overidentification. 2SLS estimates derived from different judicial characteristics should be similar assuming that judicial panel composition affect economic outcomes in the same manner regardless of the type of judicial panel. We also consider a similar set of biographical characteristics and instrumental variables for the district judges and use LASSO to identify an exogenous component of the existence of an appeal.
...
比如，在动态面板估计中，若 $T$ 较大，比如 $T>40$，则在使用 SYS-GMM 且不对工具变量的取舍做任何限制时，往往会导致在一个只有 $NT=30 \times 40 = 1200$ 个观察值的样本中，被 Stata 的 \texttt{xtabond2} 或 \texttt{xtdpdsys} 自动选入的工具变量数超过 300 个。这会导致严重的自由度损耗，也会导致后续的过度识别检验和序列相关检验无法通过 (意味着这些工具变量不全是“干净”的 IV)。虽然可以人为地限制工具变量个数，但过于主观。就模型设定角度而言，这种做法也可能遗漏一些重要的 IV，导致参数的识别度有限。
...
\begin{equation}\label{eq:lasso-iv-01}
	...
	\end{aligned}
\end{equation}
%where $\beta$ is $k \times 1$ (fixed) and $\Gamma$ is $p \times n$ with $p$ large. If $p>n$ the 2SLS estimator equals least squares. If $p<n$ but large the 2SLS estimator suffers from the "many instruments" problem. The authors' recommendation is to estimate $\Gamma$ by Lasso or post-Lasso.\footnote{%
%   As they discuss, any machine learning estimator can be used, though the specific assumptions listed in their paper are for Lasso estimation.}
其中，$\beta$ 为 $k\times 1$ (固定)，而$\Gamma$为$p\times n$。关键在于，$p$ 可以很大。如果 $p>n$，则 2SLS 估计量与 OLS 估计等价。有些情况下，虽然 $p<n$，但 $p$ 仍然较大，则2SLS估计量存在“冗余工具变量 (many instruments)”问题。此时，Lasso 就派上用场了：Belloni et al. (2012) 建议使用 Lasso 或 Post-Lasso 估计 $\Gamma$。\footnote{%
   当然，诚如作者所言，Lasso 不是唯一选择，也可以使用其它机器学习方法进行变量筛选。}
...
简言之，Lasso-IV 其实就是在传统的 2SLS 基础上增加了一个基于 Lasso 的变量筛选环节。然而，这个看似简单的步骤却可以很好地解决困扰大家已久的「冗余工具变量」问题。例如，Belloni, Chernozhukov and Hansen (\href{https://doi.org/10.1257/jep.28.2.29}{2014})
使用 Lasso-IV 方法重新估计了 Chen and Yeh (\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2977074}{2012}, \href{http://users.nber.org/~dlchen/papers/Growth_Under_the_Shadow_of_Expropriation.pdf}{PDF}) 的模型后，
发现虽然备选工具变量有 144 个，但采用 Lasso 筛选后真正有用的 IV 其实只有一个。考虑到原文的样本中只有 185 个观察值，将 IV 个数从 144 减少为 1 个，估计值的有效性将大幅提高。
%\footnote{%
%      Belloni, Chernozhukov and Hansen (\href{https://doi.org/10.1257/jep.28.2.29}{2014}) 还用 Lasso-IV 重新审视了 Acemoglu, Johnson, and Robinson (\href{https://www.aeaweb.org/articles?id=10.1257/aer.91.5.1369}{2004}), Donohue and Levitt (\href{https://doi.org/10.1162/00335530151144050}{2001}) 文中的估计结果。
%      }


\subsubsection{Stata 实操}
Stata 官方命令为 \texttt{poivregress}，采用第 \ref{sec:polasso} 小节介绍的 PO-Lasso 来执行 IV 估计。
%用以实现 Belloni et al. (2012) 以及 Belloni et al. (2014a) 文中的方法。适用于 Stata 16 以上版本的用户。 
%上述方法也可以扩展到模型中包含内生变量的情形，此时便是“偏出 IV-Lasso”估计。
详见 Chernozhukov, Hansen, and Spindler (2015a, b) 以及 \href{https://www.stata.com/manuals/lassopoivregress.pdf}{\bfseries{[\MakeUppercase{lasso}] poivregress}}, pp. 7。
%Stata 命令为 \texttt{poivregress}。 

外部命令为 \texttt{ivlasso}，用以实现  Chernozhukov, Hansen, and Spindler (2015a) 文中的估计方法，适用于 Stata 13 及以上版本。作者在帮助文件中用 Lasso-IV 重现了几篇使用传统 IV 方法的经典论文，包括 Angrist and Keueger (\href{http://sci-hub.ren/10.2307/2937954}{1991})。该命令的一大优势是附加了用以控制个体效应的选项 \texttt{fe}，可以很方便地应用到面板数据中。 

另一个外部命令是 \texttt{sivreg}，用以实现 Windmeijer et al. (\href{http://sci-hub.ren/10.1080/01621459.2018.1498346}{2019}) 文中的方法。应用该方法的论文有：Caner et al. (\href{http://sci-hub.ren/10.1016/j.iref.2021.07.010}{2021})，详见文中脚注 9 中的相关说明。

\subsection{Double/Debiased Machine Learning}\label{sec:lasso-DML}
%todo Hansen2021 29.22 Double/Debiased Machine Learning
由 Chernozhukov, Chetverikov, Demirer, Duflo, Hansen, Newey, and Robins (2018) 提出的“双重机器学习” (Double/Debiased machine learning, DML) 在近几年中获得了广泛的关注，可能是目前最拉风的估计量了。%\footnote{%
    %这篇文章的作者全部是超级大牛，本想写「最牛叉的……」，最后改成了「最拉风的……」，\texttt{\^{}=\^{}}。} 
...
作者建议用 $\widehat{\theta}_{\mathrm{DML2}}$，它也是 Stata 中 \texttt{xporegress} 命令的默认估计量。 
...
具体而言，无论是 DS-Lasso，还是 PO-Lasso 或 DML，本质上都是两阶段估计：第一阶段通过  Lasso 筛选变量 (模型筛选)，第二阶段执行 OLS 或 IV 估计。因此，如果第一阶段的估计存在较大的偏差 (比如，有些重要的变量在第一阶段未被选中)，自然会将这些偏差引入第二阶段的估计之中，从而导致第二阶段的估计结果存在较大的偏差或方差。DML 之所以能够降低这种偏误，在于它引入了交叉验证的思想，最终的估计结果其实可以视为 K 折估计的加权平均。换个角度来看，这种处理其实也借鉴了近十年来颇受关注的 “模型均化” (Model Averaging) 的思想。\footnote{%
  所谓“模型均化” (Model Averaging) 是指以多种方法得到的估计或预测结果的平均值作为最终估计量。之所以如此，是因为没有任何一种模型可以绝对优于其它模型，此时采用它们的平均值作为估计量，便成为一种次优选择。详情参见 Hansen (\href{http://sci-hub.ren/10.1111/j.1468-0262.2007.00785.x}{2007}), Moral-Benito (\href{http://sci-hub.ren/10.1111/joes.12044}{2015}), Steel (\href{http://sci-hub.ren/10.1257/jel.20191385}{2020})。}
...
Stata 提供了多种模型的 DML 估计方法，相关命令均以 \texttt{xpo} 开头，包括：\texttt{xporegress} (线性回归), \texttt{xpologit} (Logit 模型), \texttt{xpopoisson} (泊松模型), \texttt{xpoivregress} (IV 估计)，详见 \href{https://www.stata.com/manuals/lassoxporegress.pdf}{\bfseries{[\MakeUppercase{lasso}] xporegress}}。

Stata 手册以 Wooldridge (2010, ex. 5.3) 为例，使用 Mroz (\href{http://sci-hub.ren/10.2307/1911029}{1987}) 文中的数据演示了 \texttt{xpoivregress} 的使用方法。详见 
\href{https://www.stata.com/manuals/lassoinferenceexamples.pdf#lassoInferenceexamplesRemarksandexamples6Fittinganinferentialmodelwithendogenouscovariates}{xpoivregress Exmaple 6}。




\iffalse %todo ================注释-------begin----
By default it implements the DML2 estimator with $K=10$ folds. The coefficients $\gamma$ and $\eta$ are estimated by post-Lasso.

\begin{itemize}
\item \texttt{help xpoivregress}  DML, Cross-fit partialing-out lasso IV 估计
\item \texttt{help xpologit}      DML, Cross-fit partialing-out lasso logistic 回归
\item \texttt{help xpopoisson}    DML, Cross-fit partialing-out lasso Poisson 回归
\item \texttt{help xporegress}    DML, Cross-fit partialing-out lasso 线性回归) 
\end{itemize}
...
%see \href{https://www.stata.com/new-in-stata/lasso-with-clustered-data/}{Stata 17 new feature: Lasso with clustered data}
...
\section{Group Lasso}
%todo https://www.zhihu.com/question/38121173/answer/75158776
不难看出，由于 $L 1$ penalty规定的范围 “四四方方、有棱有角” ，所以最优解 $^{a}$ 的系数会被刚好缩成 零，因此lasso可以实现对变量的选择 (系数为零的变量就被垶掉了)。
有趣的是，我们还可以将所有变量分组，然后在目标函数中佂罚每一组的L范数，这样达到的效果 就是可以将一整组的系数同时消成零，即抺掉一整组的变量，这种手法叫做group lasso，其目标 函数如下:
$$
\min _{\beta} \frac{1}{2}\left\|y-\sum_{l=1}^{m} X^{(l)} \beta^{(l)}\right\|_{2}^{2}+\lambda \sum_{l=1}^{m} \sqrt{p_{l}}\left\|\beta^{(l)}\right\|_{2}
$$
其中我们把所有变量分为 $m$ 组，第一项是通常的OLS，第二项是每一组系数的 $L 2$ 范数之和。这 里， $\lambda$ 控制整体至罚的力度， $\sqrt{\rho_{l}}$ 是每一组的加权，可以按需调节。
比如一个regression若有 10 个系数 $\beta_{1}, \beta_{2}, \ldots, \beta_{10}$ ，我们如果选择将其分成2组: 其中 $\beta_{1}, \ldots, \beta_{5}$ 一组， $\beta_{6}, \ldots, \beta_{10}$ 一组。那么 group lasso的惩罚项将会是：
$$
\rho\left(\sqrt{p_{1}} \sqrt{\sum_{i=1}^{5} \beta_{i}^{2}}+\sqrt{p_{2}} \sqrt{\left.\sum_{j=6}^{10} \beta_{j}^{2}\right)}\right.
$$
通过施加group-wise ${ }^{a}$ 的L2 penalty，我们有可能促使 $\beta_{1}=\beta_{2}=\ldots=\beta_{5}=0$ 或者 $\beta_{6}=\beta_{7}=\ldots=\beta_{10}=0$.
最后，还有一种lasso和group lasso的奇妙结合，叫做 sparse group lasso $^{2}$ ，由 Simon et al. (2013) 提出，sparse group lasso的目标函数 (如下) 的惩罚项中，既有所有系数的L1范数，又有每一组系数的 $L2$ 范数，可以视为将弹性网的思路扩展到了 Group Lasso 中：
$$
\frac{1}{2 n}\left\|y-\sum_{l=1}^{m} X^{(l)} \beta^{(l)}\right\|^{2}+(1-\alpha) \lambda \sum_{l=1}^{m} \sqrt{p_{l}}\left\|\beta^{(l)}\right\|_{2}+\alpha \lambda\|\beta\|_{1}
$$

其中 $\lambda$ 依然控制总体的惩罚力度，有新引入 $\alpha$ 控制两个惩罚项之间的相互强弱。所以sparse group lasso既可以把系数和变量一组一组地施掉，又可以在剩下的组中䇠掉一些单个的系数，原埋 图如下:
当然了，这只是在简单OLS背景下的lasso、ridge、和group lasso和sparse group lasso，更常用 的目标函数的第一项一般是log likelihood ( 用于maximum likelihood手法)。相似的概念也可 以迁移到其他场景，比如因子分析模型 ( factor analysis model )，其中group lasso可以邦助进 行对可被观测的变量选取，而sparse group lasso可以选取隐藏因子，我统计的thesis做的就是这 个啦。

\section{Cluster Lasso}

%Belloni et al. ([2016](https://link.springer.com/article/10.1007/s00148-020-00778-2#ref-CR4))


应用：Qiu, Yun, Xi Chen, and Wei Shi. 2020. “Impacts of Social and Economic Factors on the Transmission of Coronavirus Disease 2019 (COVID-19) in China.” Journal of Population Economics 33 (4): 1–46. 

%The influence of weather conditions on policy adoption may be complicated, so we use the Cluster-Lasso method of Belloni et al. ([2016](https://link.springer.com/article/10.1007/s00148-020-00778-2#ref-CR4)) to select the weather variables that have good predictive power on the adoption of closed management of communities or family outdoor restrictions. 

扩展：Chiang, H. D., K. Kato, Y. Ma, Y. Sasaki (\href{https://doi.org/10.1080/07350015.2021.1895815}{2022}, \href{http://sci-hub.ren/10.1080/07350015.2021.1895815}{PDF})

Chiang, H. D., K. Kato, Y. Ma, Y. Sasaki. 2022. "Multiway cluster robust double/debiased machine learning". Journal of Business \& Economic Statistics, 40 (3): 1046-1056 Stata (crhdreg). \href{https://doi.org/10.1080/07350015.2021.1895815}{Link}, \href{https://doi.org/10.1080/07350015.2021.1895815}{Link}, \href{http://sci-hub.ren/10.1080/07350015.2021.1895815}{PDF}, \href{https://www.tandfonline.com/doi/suppl/10.1080/07350015.2021.1895815?scroll=top}{Replication}，Stata 命令为 \texttt{chrdreg}。

\subsection{fused lasso}

Tibshirani, Robert, Michael D Saunders, Saharon Rosset, Ji Zhu, and Keith Knight. 2005. “Sparsity and Smoothness via the Fused Lasso.” Journal of The Royal Statistical Society Series B-Statistical Methodology 67 (1): 91–108. [-Link-](https://academic.microsoft.com/paper/2140514146/reference), [-PDF-](http://sci-hub.ren/10.1111/J.1467-9868.2005.00490.X)
...
%todo 链接：https://www.zhihu.com/question/38121173/answer/75158776
...
$\quad \textcolor{red}{\circ}$ 
         \texttt{help lasso intro}
...
%使用 \texttt{help Lasso intro} 命令可以对 Stata 中的 Lasso 系列命令和功能有个整体认知。

%\vspace{5pt}
Stata 中执行 Lasso 估计的三条最基本的命令是:

\vspace{5pt}
$\quad \textcolor{blue}{\circ}$  
\texttt{lasso}, 
\texttt{sqrtlasso}, 
\texttt{elasticnet}
...
\vspace{5pt}
\href{https://www.stata.com/manuals/lassolasso.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso}}：Lasso 和相关命令的整体介绍 (简明易懂)；

\href{https://www.stata.com/manuals/lassolasso intro.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso intro}}：有关 Lasso 方法的 Stata 命令概览；  

\href{https://www.stata.com/manuals/lassolassofitting.pdf#lassolassofitting}{\bfseries{[\MakeUppercase{lasso}] lasso fitting}}：Lasso 的模型拟合过程：

\href{https://www.stata.com/manuals/lassolasso glossary.pdf}{\bfseries{[\MakeUppercase{lasso}] lasso glossary}}：Lasso 中的各类术语 

\href{https://www.stata.com/manuals/tetelasso.pdf}{\bfseries{[\MakeUppercase{te}] telasso}}：Lasso 因果推断，如计算 ATE, ATET, POMs 等。

\vspace{5pt}
此外，Stata 17 中的 Lasso 都能应对聚类数据 (clustered data)，这对于多数研究中使用的面板数据尤为重要。详情参见： \href{https://www.stata.com/new-in-stata/lasso-with-clustered-data/}{Stata 17 new feature: Lasso with clustered data}。

\vspace{5pt}
如下几篇文章对 Stata 中 Lasso 应用做了入门介绍，有助于快速了解 \texttt{lasso} 系列命令的主要功能。

Ahrens A., Hansen C.B., Schaffer M.E. (2020). \texttt{lassopack}: Model selection and prediction with regularized regression in Stata. The Stata Journal. 20(1):176-235. \href{https://arxiv.org/pdf/1901.05397.pdf}{PDF1}, \href{https://sci-hub.ren/10.1177/1536867X20909697}{PDF2}, \href{https://www.stata.com/meeting/uk18/}{Slides}, \href{https://statalasso.github.io/docs/lassopack/}{Github}

Drukker, D. M., and D. Liu. 2019. An introduction to the lasso in Stata. The Stata Blog.
\href{https://blog.stata.com/2019/09/09/an-introduction-to-the-lasso-in-stata/}{Link}.

Drukker, D. M., and D. Liu. 2019. Using the lasso for inference in high-dimensional models. The Stata Blog.
\href{https://blog.stata.com/2019/09/09/using-the-lasso-for-inference-in-high-dimensional-models/}{Link}
...
\vspace{10pt}
\begin{verbatim}
. help bicplot // Plot Bayesian information criterion function after lasso
...
Lasso 在经济金融领域的应用正在快速增长。例如，Belloni, Chernozhukov and Hansen (\href{https://doi.org/10.1257/jep.28.2.29}{2014}) 便应用 Lasso 重现了三篇经典论文中的关键结果，得到了不少有趣的发现。

%\noindent
Belloni, A., V. Chernozhukov, and C. B. Hansen. 2014. 
High-dimensional methods and inference on structural and treatment effects. 
Journal of Economic Perspectives 28: 29–50. \href{https://doi.org/10.1257/jep.28.2.29}{Link}, \href{http://sci-hub.ren/10.1257/jep.28.2.29}{PDF}, \href{https://academic.oup.com/restud/article/81/2/608/1523757}{Stata-codes}
\vspace{3pt}
...
\vspace{3pt}
$\textcolor{blue}{\circ}$ 
Acemoglu, D., S. Johnson, and J. A. Robinson. 2001. "The Colonial Origins of Comparative Development: An Empirical Investigation." American Economic Review 91(5): 1369-1401. \href{https://www.aeaweb.org/articles?id=10.1257/aer.91.5.1369}{Link},  \href{http://sci-hub.ren/10.1257/aer.91.5.1369}{PDF}, \href{http://economics.mit.edu/faculty/acemoglu/data/ajr2001}{replication}

$\textcolor{blue}{\circ}$ 
Chen, Daniel L., and Susan Yeh. 2012. "Growth under the Shadow of Expropriation? The Economic Impacts of Eminent Domain." \href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2977074}{Link}

$\textcolor{blue}{\circ}$ 
Donohue, John J., and Steven D. Levitt. 2001. "The Impact of Legalized Abortion on Crime." Quarterly Journal of Economics 116(2): 379-420. \href{https://doi.org/10.1162/00335530151144050}{Link}, \href{http://sci-hub.ren/10.1162/00335530151144050}{PDF} (堕胎合法化对犯罪的影响)
\href{http://sci-hub.ren/10.1162/qjec.2008.123.1.407}{Comment}, \href{http://sci-hub.ren/10.1093/aler/ahaa008}{Literature Review}

\vspace{6pt}

此外，如下文章提供了 Stata 重现代码，可以作为理解 Lasso 应用的阅读材料。

\subsubsection{Paravisini, et al. (\href{https://doi.org/10.1093/restud/rdu028}{2015}, \href{http://sci-hub.ren/10.1093/restud/rdu028}{PDF})}

Paravisini, D., V. Rappoport, P. Schnabl, D. Wolfenzon. 2015. Dissecting the effect of credit supply on trade: Evidence from matched credit-export data. The Review of Economic Studies, 82 (1): 333-359. \href{https://doi.org/10.1093/restud/rdu028}{Link}, \href{https://doi.org/10.1093/restud/rdu028}{Link}, \href{http://sci-hub.ren/10.1093/restud/rdu028}{PDF}, \href{https://academic.oup.com/restud/article/82/1/333/1545669}{Replication}

该文采用 \ref{sec:lasso-IV} 小节介绍的 Lasso-IV 研究了信贷供给对贸易行为的影响。  

\subsubsection{Myers, K. (\href{https://doi.org/10.1257/app.20180518}{2020}, \href{http://sci-hub.ren/10.1257/app.20180518}{PDF})}

Myers, K. 2020. The elasticity of science. American Economic Journal: Applied Economics, 12 (4): 103-134. \href{https://doi.org/10.1257/app.20180518}{Link}, \href{https://www.aeaweb.org/articles?id=10.1257/app.20180518}{Link}, \href{http://sci-hub.ren/10.1257/app.20180518}{PDF}, \href{https://www.aeaweb.org/doi/10.1257/app.20180518.data}{Replication}

该文使用 \texttt{pdslasso} (post-selection and post-regularization Lasso-IV，见 \ref{sec:polasso} 小节)，\texttt{rlasso} (sqrt-Lasso，见 \ref{sec:sqrt-lasso} 小节)，\texttt{ivlasso} (见 \ref{sec:lasso-IV} 小节) 等外部命令进行了 Lasso-IV 估计。
  


\section{本章小结}

Lasso 家族还有很多成员，比如：对变量进行分组惩罚的 Group Lasso、考虑群组特征的 Lasso (Lasso with clustered data)、适用于时间序列分析的 fused lasso (Tibshirani et al., \href{http://sci-hub.ren/10.1111/J.1467-9868.2005.00490.X}{2005}) 等。有兴趣的读者可以参阅第 \ref{sec:lassointro} 小节提及的文献。

一个不容置疑的趋势是，Lasso 会越来越深地融入到因果推断之中，以至于 Stata 单列了一本手册介绍相关模型： \href{https://www.stata.com/manuals/tetelasso.pdf}{\bfseries{[\MakeUppercase{te}] telasso}}。传统的估计 ATE, ATET, POMs 的方法都融入了 Lasso 基因。
...
%
%\section{HDFE + ML}
%%todo 基于 Lasso 的高维固定效应模型，待更新，2022/7/4 10:46
%
%Chiang, H. D., K. Kato, Y. Ma, Y. Sasaki (\href{https://doi.org/10.1080/07350015.2021.1895815}{2022}, \href{http://sci-hub.ren/10.1080/07350015.2021.1895815}{PDF})
...
S458459. \href{https://ideas.repec.org/c/boc/bocode/s458459.html}{Link}.

Angrist, J. D., \& Keueger, A. B. (1991). Does compulsory school attendance affect schooling and earnings?. The Quarterly Journal of Economics, 106(4), 979-1014. \href{https://doi.org/10.2307/2937954}{Link}, \href{http://sci-hub.ren/10.2307/2937954}{PDF}

Athey, S., \& Imbens, G. W. (2019). Machine learning methods that economists should know about. Annual Review of Economics, 11, 685-725. \href{https://doi.org/10.1146/annurev-economics-080217-053433}{Link}, \href{http://sci-hub.ren/10.1146/annurev-economics-080217-053433}{PDF}

Ahrens, A., C. B. Hansen, and M. E. Schaffer. 2020. lassopack: Model selection and prediction with regularized regression in Stata. Stata Journal 20: 176–235. \href{http://sci-hub.ren/10.1177/1536867X20909697}{PDF}, \href{https://statalasso.github.io/pdf/StataConference2018_AhrensSchaffer.pdf}{PPT}

Athey, S., Imbens, G. W., \& Wager, S. (2018). Approximate residual balancing: debiased inference of average treatment effects in high dimensions. Journal of the Royal Statistical Society: Series B (Statistical Methodology), 80(4), 597-623. \href{https://doi.org/10.1111/rssb.12268}{Link}, \href{http://sci-hub.ren/10.1111/rssb.12268}{PDF}         

Akaike, H. 1974. A new look at the statistical model identification.  IEEE Transactions on Automatic Control 19: 716-723. \href{https://doi.org/10.1109/TAC.1974.1100705}{Link}, \href{http://sci-hub.ren/10.1109/TAC.1974.1100705}{PDF}.

Belloni, A., D. Chen, V. Chernozhukov, and C. B. Hansen. 2012. Sparse models and methods for optimal instruments
with an application to eminent domain. Econometrica 80: 2369–2429. \href{https://doi.org/10.3982/ECTA9626}{Link}, \href{http://sci-hub.ren/10.3982/ECTA9626}{PDF} %\href{}{Stata codes}
...
Belloni, A., Chernozhukov, V., Fernández‐Val, I., \& Hansen, C. (2017). Program evaluation and causal inference with high‐dimensional data. Econometrica, 85(1), 233-298. \href{https://doi.org/10.3982/ECTA12723}{Link}, \href{http://sci-hub.ren/10.3982/ECTA12723}{PDF}

Belloni, A., V. Chernozhukov, and C. B. Hansen. 2014a. High-dimensional methods and inference on structural and
treatment effects. Journal of Economic Perspectives 28: 29–50. \href{https://doi.org/10.1257/jep.28.2.29}{Link}, \href{http://sci-hub.ren/10.1257/jep.28.2.29}{PDF}, \href{https://academic.oup.com/restud/article/81/2/608/1523757}{Stata-codes}

Belloni, A., V. Chernozhukov, and C. B. Hansen. 2014b. Inference on treatment effects after selection among high-dimensional controls. Review of Economic Studies 81: 608–650. \href{https://doi.org/10.1093/restud/rdt044}{Link}, \href{http://sci-hub.ren/10.1093/restud/rdt044}{PDF}

Belloni, A., V. Chernozhukov, and K. Kato. 2015. Uniform post-selection inference for least absolute deviation regression and other Z-estimation problems. Biometrika 102: 77-94. \href{https://doi.org/10.1093/biomet/asu056}{Link}, \href{http://sci-hub.ren/10.1093/biomet/asu056}{PDF}

Belloni, A., V. Chernozhukov, and L. Wang. 2011. Square-root lasso: Pivotal recovery of sparse signals via conic programming. Biometrika 98: 791–806. \href{https://doi.org/10.1093/biomet/asr043}{Link}, \href{http://sci-hub.ren/10.1093/biomet/asr043}{PDF}

Belloni, A., V. Chernozhukov, and Y. Wei. 2016. Post-selection inference for generalized linear models with many controls. Journal of Business \& Economic Statistics 34: 606–619.  \href{https://doi.org/10.1080/07350015.2016.1166116}{Link}, \href{http://sci-hub.ren/10.1080/07350015.2016.1166116}{PDF}


Bickel, P. J., Y. Ritov, and A. B. Tsybakov. 2009. Simultaneous analysis of Lasso and Dantzig selector. Annals of
Statistics 37: 1705–1732. \href{https://doi.org/10.1214/08-A}{Link}, \href{http://sci-hub.ren/10.1214/08-A}{PDF}

Buhlmann, P., and S. van de Geer. 2011. Statistics for High-Dimensional Data: Methods, Theory and Applications.
Berlin: Springer.

Caner, M., Fan, Q., \& Grennes, T. (2021). Partners in debt: An endogenous non-linear analysis of the effects of public and private debt on growth. International Review of Economics \& Finance, 76, 694-711. \href{https://doi.org/10.1016/j.iref.2021.07.010}{Link}, \href{http://sci-hub.ren/10.1016/j.iref.2021.07.010}{PDF}

Cerulli, G., 2021a. "Improving econometric prediction by machine learning," Applied Economics Letters, 28(16): 1419-1425. \href{http://sci-hub.ren/10.1080/13504851.2020.1820939}{PDF}

Cerulli, G., 2021b. "Machine learning using Stata/Python," 2021 Stata Conference 25, Stata Users Group. \href{https://arxiv.org/pdf/2103.03122.pdf}{PDF}

Chen, Daniel L., and Susan Yeh. 2012. "Growth under the Shadow of Expropriation? The Economic Impacts of Eminent Domain." \href{http:// www.sole-jole.org/13463.pdf}{PDF}

Chen, J., and Z. Chen. 2008. Extended Bayesian information criteria for model selection
with large model spaces. Biometrika 95: 759–771. \href{https://doi.org/10.1093/biomet/asn034}{Link}, \href{http://sci-hub.ren/10.1093/biomet/asn034}{PDF}
...
\href{https://doi.org/10.1111/ectj.12097}{Link}, \href{http://sci-hub.ren/10.1111/ectj.12097}{PDF}, \href{https://xs2.dailyheadlines.cc/scholar?cites=17289873169606420768&as_sdt=2005&sciodt=0,5&hl=zh-CN}{Cited}
...
\href{https://doi.org/10.1257/aer.p20151022}{Link}, \href{http://sci-hub.ren/10.1257/aer.p20151022}{PDF}

Chernozhukov, V., C. B. Hansen, and M. Spindler. 
2015b. Valid post-selection and post-regularization inference: An elementary, general approach. Annual Review of Economics 7: 649-688. \href{https://doi.org/10.1146/annurev-economics-012315-015826}{Link}, \href{http://sci-hub.ren/10.1146/annurev-economics-012315-015826}{PDF}


Chiang, H. D., K. Kato, Y. Ma, Y. Sasaki, 2022, Multiway cluster robust double/debiased machine learning, Journal of Business \& Economic Statistics, 40 (3): 1046-1056. \href{https://doi.org/10.1080/07350015.2021.1895815}{-Link-}, \href{https://sci-hub.ren/10.1080/07350015.2021.1895815}{-PDF-}, \href{https://www.tandfonline.com/doi/suppl/10.1080/07350015.2021.1895815?scroll=top}{Replication-R codes}, Stata: \texttt{findit crhdreg}
...
\href{https://doi.org/10.1002/cpa.20042}{Link}, \href{http://sci-hub.ren/10.1002/cpa.20042}{PDF}
...
Frisch, R., \& Waugh, F. V. (1933). Partial time regressions as compared with individual trends. Econometrica: Journal of the Econometric Society, 387-401. \href{http://sci-hub.ren/10.2307/1907330}{PDF} 

Friedman, J. H., T. J. Hastie, H. Hofling, and R. J. Tibshirani. 2007. Pathwise coordinate optimization. ¨ Annals of
Applied Statistics 1: 302–332. \href{https://doi.org/10.1214/07-AOAS131}{Link}. \href{http://sci-hub.ren/10.1214/07-AOAS131}{PDF}

Friedman, J. H., T. J. Hastie, and R. J. Tibshirani. 2010. Regularization paths for generalized linear models via
coordinate descent. Journal of Statistical Software 33: 1–22. \href{https://doi.org/10.18637/jss.v033.i01}{Link}. \href{http://sci-hub.ren/10.18637/jss.v033.i01}{PDF}

Fu, W. J. 1998. Penalized regressions: The bridge versus the lasso. Journal of Computational and Graphical Statistics
7: 397–416. \href{https://doi.org/10.1080/10618600.1998.10474784}{Link}. \href{http://sci-hub.ren/10.1080/10618600.1998.10474784}{PDF}

Hansen B E . 2021. Econometrics. Princeton University Press, forthcoming. \href{https://www.ssc.wisc.edu/~bhansen/econometrics/}{Data and Contents}, \href{https://www.ssc.wisc.edu/~bhansen/econometrics/Econometrics.pdf}{PDF}

Hastie, T. J., R. J. Tibshirani, and M. Wainwright. 2015. Statistical Learning with Sparsity: The Lasso and
Generalizations. Boca Raton, FL: CRC Press. \href{https://web.stanford.edu/~hastie/StatLearnSparsity/}{Link}. \href{https://web.stanford.edu/~hastie/StatLearnSparsity_files/SLS.pdf}{PDF}

Leeb, H., and B. M. Potscher. 2008. Sparse estimators and the oracle property, or the return of Hodges’ estimator. ¨
Journal of Econometrics 142: 201–211. \href{https://doi.org/10.1016/j.jeconom.2007.05.017}{Link} \href{http://sci-hub.ren/10.1016/j.jeconom.2007.05.017}{PDF}

Lovell, M. C. (1963). Seasonal adjustment of economic time series and multiple regression analysis. Journal of the American Statistical Association, 58(304), 993-1010. \href{http://sci-hub.ren/10.1080/01621459.1963.10480682}{PDF}
...
assumptions. Econometrica 55: 765–799. \href{https://doi.org/10.2307/1911029}{Link}, \href{http://sci-hub.ren/10.2307/1911029}{PDF}


Nelder, J. A., and R. W. M. Wedderburn. 1972. Generalized linear models. Journal of the Royal Statistical Society,
Series A 135: 370–384. \href{https://doi.org/10.2307/2344614}{Link}. \href{http://sci-hub.ren/10.2307/2344614}{PDF}
...
Epidemiology 28: 181–189. \href{https://doi.org/10.1097/EDE.0000000000000603}{Link}.
\href{http://sci-hub.ren/10.1097/EDE.0000000000000603}{PDF}

Quandt, R. E. 1984. Computational problems and methods. In Vol. 2 of Handbook of Econometrics, ed. Z. Griliches
and M. D. Intriligator, 699–764. Amsterdam: Elsevier. \href{https://doi.org/10.1016/S1573-4412(83)01016-8}{Link}. \href{http://sci-hub.ren/10.1016/S1573-4412(83)01016-8}{PDF}

Tibshirani, R. J. 1996. Regression shrinkage and selection via the lasso. Journal of the Royal Statistical Society,
Series B 58: 267–288. \href{https://doi.org/10.1111/j.2517-6161.1996.tb02080.x}{Link}. \href{http://sci-hub.ren/10.1111/j.2517-6161.1996.tb02080.x}{PDF}

Tibshirani, Robert, Michael D Saunders, Saharon Rosset, Ji Zhu, and Keith Knight. 2005. “Sparsity and Smoothness via the Fused Lasso.” Journal of The Royal Statistical Society Series B - Statistical Methodology 67 (1): 91–108. \href{https://doi.org/10.1111/J.1467-9868.2005.00490.X}{-Link-}, \href{http://sci-hub.ren/10.1111/J.1467-9868.2005.00490.X}{-PDF-}

Windmeijer, F., Farbmacher, H., Davies, N., \& Davey Smith, G. (2019). On the use of the lasso for instrumental variables estimation with some invalid instruments. Journal of the American Statistical Association, 114(527), 1339-1350. \href{https://doi.org/10.1080/01621459.2018.1498346}{Link}, \href{http://sci-hub.ren/10.1080/01621459.2018.1498346}{PDF}
...
Yamada, H. 2017.  The Frisch-Waugh-Lovell Theorem for the lasso and the ridge regression.  Communications in Statistics - Theory and Methods 46(21): 10897-10902. \href{http://dx.doi.org/10.1080/03610926.2016.1252403}{Link}, \href{http://sci-hub.ren/10.1080/03610926.2016.1252403}{PDF}.

Yamaka, W. 2021. Variable selection and estimation in kink regression model, in N. Ngoc Thach, V. Kreinovich,N. D. Trung eds, Data science for financial econometrics (Springer International Publishing, Cham), 151-164. \href{https://doi.org/10.1007/978-3-030-48853-6_11}{Link}, \href{http://sci-hub.ren/10.1007/978-3-030-48853-6_11}{PDF}.

Zhang, Y., R. Li, and C.-L. Tsai. 2010. Regularization parameter selections via generalized information criterion.
Journal of the American Statistical Association 105: 312–323. \href{https://doi.org/10.1198/jasa.2009.tm08013}{Link}. \href{http://sci-hub.ren/10.1198/jasa.2009.tm08013}{PDF}


Zhao P, Yu B. On model selection consistency of Lasso[J]. The Journal of Machine Learning Research, 2006, 7: 2541-2563. \href{https://www.jmlr.org/papers/volume7/zhao06a/zhao06a.pdf}{PDF}

Zou, H. 2006. The adaptive lasso and its oracle properties. Journal of the American Statistical Association 101:
1418–1429. \href{https://doi.org/10.1198/016214506000000735}{Link}. \href{http://sci-hub.ren/10.1198/016214506000000735}{PDF}

Zou, H., T. Hastie, and R. Tibshirani. 2007. On the “degrees of freedom” of the lasso.
Annals of Statistics 35: 2173–2192. \href{https://doi.org/10.1214/009053607000000127}{Link}, \href{http://sci-hub.ren/10.1214/009053607000000127}{PDF}

Zou, H., and H. H. Zhang. 2009. On the adaptive elastic-net with a diverging number of
parameters. Annals of Statistics 37: 1733–1751. \href{https://doi.org/10.1214/08-AOS625}{Link}, \href{http://sci-hub.ren/10.1214/08-AOS625}{PDF}
