...
先从一个多数读者都能理解的例子开始。对于 $y = x\beta + \varepsilon$，一个基本的假设是 $x$ 与 $\varepsilon$ 不相关（OLS 的正交性条件），可以表示为 $E[x\varepsilon] = 0$。当该假设无法满足时，我们就需要找到新的正交条件，比如 $E[z\varepsilon] = 0$，其中 $z$ 是一个工具变量（IV）。本例中，$E[x\varepsilon] = 0$ 和 $E[z\varepsilon] = 0$ 都是矩条件（moment conditions），它们告诉我们「在特定的假设条件下，我们设定的模型中，某个函数的期望值应该等于零」。有了矩条件，我们就可以把它们转化为样本矩条件 $\bar{g}(\theta) = \frac{1}{n} \sum_{i=1}^n g(y_i, x_i, \theta)$，然后找到使这些样本矩条件「尽可能接近零」的参数值 $\hat{\theta}$。这就是矩估计 (MM) 的核心思想。GMM 则是在 MM 的基础上引入了一个权重矩阵 $W$，通过加权最小化样本矩条件的偏离来获得更有效的估计量。

简言之，GMM 核心思想是——用「经济理论或统计假设告诉我们应当成立的矩条件」约束参数估计，找出使样本矩条件偏离最小的那组参数值。这非常类似于解方程，我们知道方程应该满足某些条件（矩条件），但由于样本的随机性，我们只能找到一个「最接近满足这些条件」的解。相比于 MLE 需要完整指定分布，GMM 只需要知道哪些矩条件应该成立，这使得它在处理复杂数据和模型时具有更大的灵活性。

---

接着提供一个进阶的例子，放在 callout-note 中，作为扩展阅读 (也可以考虑把这个例子放在好后面去，因为对于不熟悉跨期消费的读者而言，这个例子还是有一定的难度的)：

'''
从一个金融研究者面临的真实困境开始：

> 你在研究一个经典问题：投资者的跨期消费决策是否符合理性预期？理论模型告诉你，均衡时效用函数的一阶条件必须成立——这是一个关于消费增长率和资产收益率的非线性方程。你想用数据来估计效用函数的参数（贴现因子 $\beta$ 和风险厌恶系数 $\gamma$），但你不知道消费增长率服从什么分布，更不知道它与收益率的联合分布。MLE 需要你完整指定这个分布，而你做不到。怎么办？

紧接着，提出 GMM 的核心洞察：你不需要知道完整的分布，只需要知道一件事——**如果模型成立，某个函数的期望值应该等于零**。这就是矩条件。GMM 的工作，就是找到那组参数，使样本数据「最符合」这个理论预期。
'''

**B. 本章定位**

- 引出全章核心主线图（引用 `fig01_flowchart`）
- 说明本章与 MLE 章节的关系：两种估计框架各有所长，本章末尾会给出系统比较
